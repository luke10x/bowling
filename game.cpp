...
static inline void Chest_HandleTapToOpen(UserContext *usr)
{
    ...
    if (wrapped < 0.0f)
        wrapped += tau;
    const float faceCameraYaw = glm::pi<float>();
    float deltaToFace = std::fmod(faceCameraYaw - wrapped + tau, tau);
    if (deltaToFace < 0.0f)
        deltaToFace += tau;
    usr->chestRewardAlignStartYaw = usr->chestRewardYaw;
    usr->chestRewardAlignTargetYaw = usr->chestRewardYaw + deltaToFace;
...
static inline int Chest_HingeBoneIndex()
{
    if (!gChestAnimReady || !gChestAnim.anim.header)
        return -1;
    const uint32_t n = gChestAnim.anim.header->boneCount;
    for (uint32_t i = 0; i < n; ++i)
    {
        const char *nm = animNameCStr(gChestAnim.anim.bones[i].name);
        if (nm && std::strncmp(nm, "Bone.001", 64) == 0)
            return (int)i;
    }
    return -1;
}
...
static inline glm::vec3 Chest_CurrentPayoutSourcePos(const UserContext *usr)
{
    return Chest_CurrentRewardPos(usr) + glm::vec3(0.0f, -0.36f, 0.0f);
}
...
static inline glm::vec3 Chest_CurrentHingeWorldPos(const UserContext *usr)
{
    const int hingeBone = Chest_HingeBoneIndex();
    if (!usr || hingeBone < 0 || hingeBone >= (int)gChestAnim.globalMatrices.size())
        return Chest_CurrentRewardPos(usr);

    const glm::vec3 hingeModel = glm::vec3(gChestAnim.globalMatrices[(size_t)hingeBone][3]);
    const glm::mat4 chestModel = ChestRender::ModelAt(
        Chest_CurrentRewardPos(usr),
        usr->chestRewardYaw,
        Chest_CurrentRewardScale(usr)
    );
    return glm::vec3(chestModel * glm::vec4(hingeModel, 1.0f));
}
...
static inline void EnterTracker(UserContext *usr)
{
    ...
    usr->windowStack.count = 0;
    SDL_SetRelativeMouseMode(SDL_FALSE);
    const int selectedSongIndex = usr->sound.currentSongIndex;
    const bool selectedSongIsUser = (selectedSongIndex == TRACKER_USER_SONG_SLOT);
    if (selectedSongIsUser)
    {
        const char *currentSongPattern = usr->sound.getSongPattern(selectedSongIndex);
        const char *currentSongName = usr->sound.getSongName(selectedSongIndex);
    ...
    {
        Tracker_LoadSong(&usr->tracker, selectedSongIndex);
...
static inline void ExitTracker(UserContext *usr)
{
    ...
    Tracker_SaveCustomSongToStorage(usr);
...
static inline const char *Tracker_CustomSongStorageFilename()
{
...
static inline bool Tracker_WriteCustomSongText(UserContext *usr, const std::string &fileText)
{
...
static inline bool Tracker_SaveCustomSongToStorage(UserContext *usr)
{
    if (!usr || usr->sound.currentSongIndex != TRACKER_USER_SONG_SLOT)
        return false;
...
static inline bool Tracker_LoadCustomSongFromStorage(UserContext *usr)
{
    ...
    (void)TrackerSongIO_ExtractInstrumentText(fileText, instruments);
    setTrackerPatternState(
...
static inline void Tracker_LoadEmptyUserSong(UserContext *usr)
{
    ...
    std::string instrumentsText = Tracker_BuildCustomInstrumentText(&usr->trackerLoadScratch);
    ...
    usr->sound.setUserSong(
        ...
        usr->trackerLoadScratch.songTickRate,
        usr->trackerLoadScratch.songSpeed,
        usr->trackerLoadScratch.songRowsPerBeat,
...
static inline void Tracker_ApplySongNameKeypadResult(UserContext *usr)
{
    ...
    int scaleRoot = usr->tracker.songScaleRoot;
...
void vtx::loop(vtx::VertexContext *ctx)
{
    ...
    if (gameplayWholeSeconds != usr->gameplayTimeLastSavedSecond)
        Progress_SaveGameplayTime(usr);
    if (usr->sound.currentSongIndex != usr->selectedSongLastSaved)
        Progress_SaveSelectedSong(usr);
...
END_LINE:
    ...
    ZONE("clay")
    {
        if (!trackerOnlyMode && Chest_IsTextureActive(usr))
        {
            ...
            usr->chestRewardTex.ensureSize(framebufferWidth, framebufferHeight, true);
            usr->clayton.renderer.imageTextures[3] = usr->chestRewardTex.colorTexture;

            usr->chestRewardTex.bindForWriting();
            glDisable(GL_SCISSOR_TEST);
            glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
            glUseProgram(usr->mainShader.id);
            glDisable(GL_BLEND);
            ...
            usr->mainShader.updateUseTextureAlpha(false);
            ChestRender::ApplyEverythingAtlasParams(usr->mainShader);
            Chest_UploadPose(usr, "Open", Chest_CurrentOpenClipTime(usr));
            usr->mainShader.renderRealMesh(
                usr->chestMesh,
                ChestRender::ModelAt(
                    Chest_CurrentRewardPos(usr),
    ...
    if (Chest_IsTextureActive(usr))
    {
        CLAY(
            CLAY_ID("ChestRewardTexture"),
            {
                ...
                .image = {.imageData = &usr->clayton.chestRewardImage},
                .floating = {
                    .offset = {0},
                    .zIndex = 95,
                    .attachPoints = {CLAY_ATTACH_POINT_CENTER_CENTER, CLAY_ATTACH_POINT_CENTER_CENTER},
    ...
    if (Chest_IsCinematicActive(usr))
    {
        if (usr->chestCollectiblePhase == ChestRender::CollectiblePhase::WaitingTap)
        {
            ...
            CLAY(
                CLAY_ID("ChestRewardTapPrompt"),
                {
                    .layout = {
                        .sizing = {CLAY_SIZING_PERCENT(0.65f), CLAY_SIZING_PERCENT(0.125f)},
                        .childAlignment = {.x = CLAY_ALIGN_X_CENTER, .y = CLAY_ALIGN_Y_CENTER},
                    },
                    .floating = {
                        .offset = tapPromptOffset,
                        .zIndex = 96,
                        .attachPoints = {
                            .element = CLAY_ATTACH_POINT_CENTER_CENTER,
                            .parent = CLAY_ATTACH_POINT_CENTER_TOP
