#pragma once

// Generated by scripts/generate_txl.py

enum TxlKey
{
    TXL_MENU,
    TXL_RENAME,
    TXL_GO_TO_SCHOOL,
    TXL_LANGUAGE,
    TXL_CAMPAIGN,
    TXL_PRACTICE,
    TXL_FREESTYLE,
    TXL_DEVICE_SHARE,
    TXL_GAME_SETTINGS,
    TXL_TRACKER,
    TXL_CREDITS,
    TXL_MINIGAMES,
    TXL_SELECT_LANGUAGE,
    TXL_LANGUAGE_ENGLISH,
    TXL_LANGUAGE_CHINESE,
    TXL_LANGUAGE_LITHUANIAN,
    TXL_LANGUAGE_JAPANESE,
    TXL_SOUND,
    TXL_SONG,
    TXL_OIL,
    TXL_BALLS,
    TXL_SHOP,
    TXL_HI_SCORE,
    TXL_PRACTICE_TITLE,
    TXL_PRACTICE_SUBTITLE,
    TXL_FREESTYLE_TITLE,
    TXL_FREESTYLE_SUBTITLE,
    TXL_NEXT_LEVEL,
    TXL_TRY_AGAIN,
    TXL_BONUS_GAME,
    TXL_BALL_CHARGE_FMT,
    TXL_GREETING_RESUME,
    TXL_GREETING_WELCOME,
    TXL_GREETING_SUBLINE,
    TXL_READY,
    TXL_SELECT_ANGEL,
    TXL_SELECT_BOT,
    TXL_SELECT_HOUSE,
    TXL_SWITCH_HOUSE,
    TXL_SELECT_BALL,
    TXL_INVENTORY,
    TXL_BUY_NOW,
    TXL_HOUSES,
    TXL_SELECTED_FMT,
    TXL_SELECT_A_HOUSE,
    TXL_NO_HOUSES,
    TXL_BOT_AVATAR,
    TXL_SELECT_A_BOT_AVATAR,
    TXL_NO_BOTS,
    TXL_INVENTORY_EMPTY,
    TXL_SHOP_EMPTY,
    TXL_SHOP_IMPROVE_YOUR_RUN,
    TXL_CURRENT_BALANCE,
    TXL_LOCKED,
    TXL_RESETS_IN_FMT,
    TXL_SOUND_SETTINGS,
    TXL_CHANGING_QUALITY,
    TXL_AUDIO_MODE,
    TXL_SDL_BUFFER,
    TXL_MUSIC_VOLUME,
    TXL_LOW_PERFORMANCE_DETECTED,
    TXL_PLEASE_CHOOSE_OPTION,
    TXL_USE_SYNTH,
    TXL_USE_CACHED,
    TXL_DISABLE,
    TXL_SYNTH_EXPLAIN,
    TXL_CACHED_EXPLAIN,
    TXL_CACHING_AUDIO,
    TXL_PROGRESS_FMT,
    TXL_OIL_STATUS,
    TXL_LANE_OIL_MAP,
    TXL_TRACK_INFO,
    TXL_MAX_OIL_LEVEL_FMT,
    TXL_SLIPPERY,
    TXL_REOIL_FREE,
    TXL_REOIL_COST_FMT,
    TXL_REOILS_FMT,
    TXL_REOIL,
    TXL_CANT_AFFORD,
    TXL_REOIL_LOCKED,
    TXL_FRESHLY_OILED,
    TXL_TOP_SCORES,
    TXL_YOUR_SCORE_TOP_FMT,
    TXL_GOOD_RUN,
    TXL_POINTS_FMT,
    TXL_SCORE_HIGHER_THAN_FMT,
    TXL_LEADERBOARD_LAST_HOUR,
    TXL_PLAYER,
    TXL_SCORE,
    TXL_AGE,
    TXL_NO_SCORES_YET,
    TXL_ATTEMPTS_FMT,
    TXL_RANGE_FMT,
    TXL_GAME_SETTINGS_TITLE,
    TXL_SNOWFLAKES_FMT,
    TXL_SNOWFLAKE_DENSITY,
    TXL_SNOW_DISABLED,
    TXL_RESET_PROGRESS,
    TXL_RESET_PROGRESS_HELP,
    TXL_CHECK_FOR_UPDATE,
    TXL_UPDATE_PWA,
    TXL_UPDATE_WEB,
    TXL_PWA_UPDATE_INSTALLED_FMT,
    TXL_PWA_UPDATE_PUBLISHED_FMT,
    TXL_PWA_UPDATE_CHECKING,
    TXL_PWA_UPDATE_UP_TO_DATE_FMT,
    TXL_PWA_UPDATE_AVAILABLE_FMT,
    TXL_PWA_UPDATE_OFFLINE,
    TXL_PWA_UPDATE_ERROR,
    TXL_PWA_UPDATE_APPLYING,
    TXL_PWA_UPDATE_UNSUPPORTED,
    TXL_CREDITS_AUTHOR_FMT,
    TXL_CREDITS_AUTHOR_NAME,
    TXL_CREDITS_CHARACTER_BASE,
    TXL_CREDITS_MIXAMO,
    TXL_CREDITS_TECH,
    TXL_CREDITS_THANKS,
    TXL_SCHOOL_TITLE_FMT,
    TXL_AIM_LESSON,
    TXL_BALL_MASS,
    TXL_SPIN_BALL,
    TXL_OIL_AND_SKID,
    TXL_STRIKE_LINE,
    TXL_MASS_FMT,
    TXL_LIGHT_BALL_TEST,
    TXL_HEAVY_BALL_TEST,
    TXL_PASSED,
    TXL_HITS_FMT,
    TXL_MASS,
    TXL_SPIN,
    TXL_SKID,
    TXL_BITE,
    TXL_RARITY_COMMON,
    TXL_RARITY_RARE,
    TXL_RARITY_EPIC,
    TXL_RARITY_LEGENDARY,
    TXL_SPIN_TEST_FMT,
    TXL_AIM_LESSON_POINTS_FMT,
    TXL_PULL_BALL_BACK,
    TXL_LET_IT_GO_NOW,
    TXL_MOVE_RIGHT,
    TXL_MOVE_LEFT,
    TXL_OIL_TEST_REOILS_FMT,
    TXL_REMAINING_OIL,
    TXL_REOIL_PROGRESS,
    TXL_WEAR_IT_DOWN,
    TXL_STRIKE_TEST_SCORE_STRIKE,
    TXL_LEVEL1_TITLE,
    TXL_LEVEL1_SUBTITLE,
    TXL_LEVEL2_TITLE,
    TXL_LEVEL2_SUBTITLE,
    TXL_LEVEL3_TITLE,
    TXL_LEVEL3_SUBTITLE,
    TXL_LEVEL4_TITLE,
    TXL_LEVEL4_SUBTITLE,
    TXL_LEVEL5_TITLE,
    TXL_LEVEL5_SUBTITLE,
    TXL_LEVEL6_TITLE,
    TXL_LEVEL6_SUBTITLE,
    TXL_LEVEL7_TITLE,
    TXL_LEVEL7_SUBTITLE,
    TXL_LEVEL8_TITLE,
    TXL_LEVEL8_SUBTITLE,
    TXL_LEVEL9_TITLE,
    TXL_LEVEL9_SUBTITLE,
    TXL_LEVEL10_TITLE,
    TXL_LEVEL10_SUBTITLE,
    TXL_LEVEL11_TITLE,
    TXL_LEVEL11_SUBTITLE,
    TXL_LEVEL12_TITLE,
    TXL_LEVEL12_SUBTITLE,
    TXL_LEVEL13_TITLE,
    TXL_LEVEL13_SUBTITLE,
    TXL_BALL_NAME_0,
    TXL_BALL_NAME_1,
    TXL_BALL_NAME_2,
    TXL_BALL_NAME_3,
    TXL_BALL_NAME_4,
    TXL_BALL_NAME_5,
    TXL_BALL_NAME_6,
    TXL_BALL_NAME_7,
    TXL_BALL_NAME_8,
    TXL_BALL_NAME_9,
    TXL_BALL_NAME_10,
    TXL_BALL_NAME_11,
    TXL_BALL_NAME_12,
    TXL_BALL_NAME_13,
    TXL_BALL_NAME_14,
    TXL_BALL_NAME_15,
    TXL_BALL_NAME_16,
    TXL_BALL_NAME_17,
    TXL_BALL_NAME_18,
    TXL_BALL_NAME_19,
    TXL_BALL_NAME_20,
    TXL_BALL_NAME_21,
    TXL_BALL_NAME_22,
    TXL_BALL_NAME_23,
    TXL_BALL_NAME_24,
    TXL_BALL_NAME_25,
    TXL_BALL_NAME_26,
    TXL_BALL_NAME_27,
    TXL_BALL_NAME_28,
    TXL_BALL_NAME_29,
    TXL_BALL_NAME_30,
    TXL_BALL_NAME_31,
    TXL_BALL_NAME_32,
    TXL_BALL_NAME_33,
    TXL_BALL_NAME_34,
    TXL_KEY_COUNT
};

static constexpr const char *g_txl_en_us[TXL_KEY_COUNT] = {
    "Menu",
    "Cheats",
    "Go to school",
    "Language",
    "Play",
    "Practice",
    "Freestyle",
    "Device Share",
    "Game Settings",
    "Tracker",
    "Credits",
    "MINIGAMES",
    "Select Language",
    "English",
    "Chinese",
    "Lithuanian",
    "Japanese",
    "Sound",
    "Song",
    "Oil",
    "BALLS",
    "Shop",
    "Hi-Score",
    "PRACTICE",
    "House and ball selection run",
    "FREESTYLE",
    "Versus with your unlocked angel, house, and ball",
    "NEXT LEVEL",
    "TRY AGAIN",
    "BONUS GAME",
    "ENERGY",
    "Do you want to continue to play bowling?",
    "Welcome to this bowling game.",
    "Click READY to focus the game canvas.",
    "READY",
    "SELECT ANGEL",
    "SELECT BOT",
    "SELECT HOUSE",
    "SWITCH HOUSE",
    "SELECT BALL",
    "INVENTORY",
    "BUY NOW",
    "Houses",
    "Selected: %s",
    "Select a house",
    "No houses",
    "Bot Avatar",
    "Select a bot avatar",
    "No bots",
    "INVENTORY IS EMPTY",
    "SHOP IS EMPTY",
    "SHOP: IMPROVE YOUR RUN",
    "Current balance",
    "LOCKED",
    "Resets in %s",
    "Sound Settings",
    "Changing quality...",
    "Audio Mode",
    "SDL Buffer",
    "Music Volume",
    "Low Performance Detected",
    "Please choose an option:",
    "Use Synth",
    "Use Cached",
    "Disable",
    "Synth: Real-time OPN chip synthesis (no preload, more CPU)",
    "Cached: Pre-generated audio blobs (needs caching, lighter on CPU)",
    "Caching Audio...",
    "Progress: %d%% (%.1fs / %.1fs)",
    "Oil Status",
    "Lane Oil Map",
    "Track Info",
    "Max oil level: %.1fmm\nCurrent: %.1fmm\nCarrydown: %.3fm/m\nDecay: %.4f/m",
    "Slippery",
    "Re-oil: FREE",
    "Re-oil cost: $%.0f  (you have: $%.0f)",
    "Re-oils: %d/%d",
    "RE-OIL",
    "CAN'T AFFORD",
    "RE-OIL LOCKED",
    "FRESHLY OILED",
    "Top Scores",
    "Your score is in top %d",
    "Good Run!",
    "%d points",
    "Your score is higher than %.0f%% of all recent runs",
    "Leaderboard (Last Hour)",
    "Player",
    "Score",
    "Age",
    "No scores yet - be the first!",
    "Attempts: %d",
    "Range: %d-%d",
    "Game Settings",
    "Snowflakes: %d / %d",
    "Snowflake density",
    "Snow is fully disabled, including updates and draw calls.",
    "RESET PROGRESS",
    "Resets campaign level, unlocks, and cash. Username stays unchanged.",
    "CHECK FOR UPDATE",
    "UPDATE PWA",
    "RELOAD WEBSITE",
    "Installed build: %s",
    "Published build: %s",
    "Checking for update...",
    "Up to date. Published build: %s",
    "Update available: %s -> %s",
    "Offline. Connect to the internet to check for updates.",
    "Could not reach the update server.",
    "Applying update...",
    "PWA update checks are only available in the web app.",
    "Game design, programming, art, and sound by %s.",
    "Lapė Kalė",
    "Character base: Very Low Poly Human from OpenGameArt.org.",
    "Character animations from Mixamo.",
    "Built with SDL, OpenGL ES, Clay, GLM, Jolt Physics, and ymfm.",
    "Special thanks to everyone who played, tested, and kept throwing another frame.",
    "Lesson %d. %s",
    "Aim lesson",
    "Ball Mass",
    "Spin ball",
    "Oil and skid",
    "Strike line",
    "MASS (%.1fKG)",
    "Light ball test",
    "Heavy ball test",
    "Passed",
    "%d / %d hits",
    "MASS",
    "SPIN",
    "SKID",
    "BITE",
    "COMMON",
    "RARE",
    "EPIC",
    "LEGENDARY",
    "Spin test: %d/%d targets (Level %d/%d)",
    "Aim lesson: %d/%d points",
    "Pull the ball all the way back",
    "Let it go now",
    "Move right",
    "Move left",
    "Oil test: re-oils %d/%d",
    "Remaining oil",
    "Re-oil progress",
    "Wear it down before re-oiling.",
    "Strike test: score a STRIKE",
    "LEVEL 1  FIRST MILESTONE",
    "Normal biome  Reach 100 to pass",
    "LEVEL 2  MALACH ARRIVES",
    "Normal biome  Beat Malach",
    "LEVEL 3  DESERT WARNING",
    "Desert biome  Beat Malach",
    "LEVEL 4  GLASS ICE",
    "Ice biome  Beat Malach",
    "LEVEL 5  NEON GLASS CLASS",
    "Neon biome  Beat Malach",
    "LEVEL 6  CHERUBEL IN NEON",
    "Neon biome  Beat Cherubel",
    "LEVEL 7  POWER SHOT CLASS",
    "Normal biome  Beat Cherubel",
    "LEVEL 8  SAND TIMBER",
    "Desert biome  Beat Cherubel",
    "LEVEL 9  SERAPHEL IN THE DUNES",
    "Desert biome  Beat Seraphel",
    "LEVEL 10  ICE AUDIENCE",
    "Ice biome  Beat Seraphel",
    "LEVEL 11  BRICK CONFESSION",
    "Neon biome  Beat Seraphel",
    "LEVEL 12  WHEELS OF THE CITY",
    "Neon biome  Beat Thrones",
    "LEVEL 13  CONCRETE PARADE",
    "Normal biome  Beat Thrones",
    "Ember Strike",
    "Flame Roll",
    "Blaze Hook",
    "Inferno Fury",
    "Phoenix Rise",
    "Frost Glide",
    "Chill Strike",
    "Arctic Drift",
    "Glacier Bite",
    "Blizzard King",
    "Star Dust",
    "Nebula Roll",
    "Cosmic Hook",
    "Void Strike",
    "Black Hole",
    "UFO Spin",
    "Xenon Bite",
    "Forest Roll",
    "Vine Strike",
    "Thunder Oak",
    "Titan Root",
    "World Tree",
    "Shadow Drift",
    "Abyss Hook",
    "Nullifier",
    "Circuit Roll",
    "Neon Strike",
    "Quantum Hook",
    "Singularity",
    "Tide Roll",
    "Reef Strike",
    "Kraken Bite",
    "Leviathan",
    "Rune Ball",
    "Oracle Strike",
};

static constexpr const char *g_txl_chars_en_us = "MenuChatsGo clLgPyriFDvSmTkdINAEJpOBH-RbYVw,XU?W.f:%Kq()1/x\n34$0'!>ė256789zQ";

static constexpr const char *g_txl_lt_lt[TXL_KEY_COUNT] = {
    "Meniu",
    "Sukčiavimai",
    "Eiti į mokyklą",
    "Kalba",
    "Žaisti",
    "Praktika",
    "Laisvas stilius",
    "Įrenginio bendrinimas",
    "Žaidimo nustatymai",
    "Garso takelių redaktorius",
    "Kreditai",
    "Minižaidimai",
    "Pasirinkti kalbą",
    "Anglų",
    "Kinų",
    "Lietuvių",
    "Japonų",
    "Garsas",
    "Daina",
    "Alyva",
    "KAMUOLIAI",
    "Parduotuvė",
    "Aukščiausias rezultatas",
    "PRAKTIKA",
    "Tako ir kamuolio pasirinkimo žaidimas",
    "LAISVAS STILIUS",
    "Kova su atrakintais angelu, taku ir kamuoliu",
    "KITAS LYGIS",
    "BANDYTI DAR KARTĄ",
    "BONUSO ŽAIDIMAS",
    "ENERGIJA",
    "Ar norite tęsti boulingą?",
    "Sveiki atvykę į šį boulingo žaidimą.",
    "Spustelėkite PARUOŠTA, kad sufokusuotumėte žaidimo drobę.",
    "PARUOŠTA",
    "PASIRINKTI ANGELĄ",
    "PASIRINKTI BOTĄ",
    "PASIRINKTI TAKĄ",
    "KEISTI TAKĄ",
    "PASIRINKTI KAMUOLĮ",
    "INVENTORIUS",
    "PIRKTI DABAR",
    "Takai",
    "Pasirinkta: %s",
    "Pasirinkite taką",
    "Nėra takų",
    "Boto avataras",
    "Pasirinkite boto avatarą",
    "Nėra botų",
    "INVENTORIUS TUŠČIAS",
    "PARDUOTUVĖ TUŠČIA",
    "PARDUOTUVĖ: PAGERINK SAVO METIMĄ",
    "Dabartinis likutis",
    "UŽRAKINTA",
    "Atsistatys po %s",
    "Garso nustatymai",
    "Keičiama kokybė...",
    "Garso režimas",
    "SDL buferis",
    "Muzikos garsumas",
    "Aptiktas žemas našumas",
    "Pasirinkite parinktį:",
    "Naudoti sintezę",
    "Naudoti podėlį",
    "Išjungti",
    "Sintezė: Realaus laiko OPN lusto sintezė (be išankstinio įkėlimo, didesnė procesoriaus apkrova)",
    "Podėlis: Iš anksto sugeneruoti garso failai (reikia podėlio, mažesnė procesoriaus apkrova)",
    "Įrašoma į podėlį...",
    "Progresas: %d%% (%.1fs / %.1fs)",
    "Alyvos būsena",
    "Tako alyvos žemėlapis",
    "Tako informacija",
    "Maks. alyvos lygis: %.1fmm\nDabartinis: %.1fmm\nPernešimas: %.3fm/m\nSkilimas: %.4f/m",
    "Slidu",
    "Pertepti: NEMOKAMAI",
    "Pertepimo kaina: $%.0f (turite: $%.0f)",
    "Pertepimai: %d/%d",
    "PERTEPTI",
    "Nepakanka lėšų",
    "PERTEPIMAS UŽRAKINTAS",
    "ŠVIEŽIAI PERTEPTA",
    "Geriausi rezultatai",
    "Jūsų rezultatas tarp %d geriausių",
    "Puikus žaidimas!",
    "%d taškai",
    "Jūsų rezultatas didesnis nei %.0f%% visų pastarųjų žaidimų",
    "Lyderių lentelė (paskutinė valanda)",
    "Žaidėjas",
    "Rezultatas",
    "Laikas",
    "Dar nėra rezultatų – būk pirmas!",
    "Bandymai: %d",
    "Diapazonas: %d–%d",
    "Žaidimo nustatymai",
    "Snaigės: %d / %d",
    "Snaigių tankis",
    "Sniegas visiškai išjungtas, įskaitant atnaujinimus ir piešimo iškvietimus.",
    "ATSTATYTI PROGRESĄ",
    "Atstato kampanijos lygį, atrakintus dalykus ir pinigus. Naudotojo vardas lieka nepakitęs.",
    "TIKRINTI ATNAUJINIMĄ",
    "ATNAUJINTI PWA",
    "IŠ NAUJO ĮKELTI SVETAINĘ",
    "Įdiegta versija: %s",
    "Paskelbta versija: %s",
    "Tikrinamas atnaujinimas...",
    "Naujausia versija jau įdiegta. Paskelbta versija: %s",
    "Yra atnaujinimas: %s -> %s",
    "Esate neprisijungę. Prisijunkite prie interneto ir bandykite dar kartą.",
    "Nepavyko pasiekti atnaujinimų serverio.",
    "Taikomas atnaujinimas...",
    "PWA atnaujinimų tikrinimas galimas tik žiniatinklio programoje.",
    "Žaidimo dizainas, programavimas, menas ir garsas: %s.",
    "Lapė Kalė",
    "Personažo pagrindas: Very Low Poly Human iš OpenGameArt.org.",
    "Personažų animacijos iš Mixamo.",
    "Sukurta naudojant SDL, OpenGL ES, Clay, GLM, Jolt Physics ir ymfm.",
    "Ypatinga padėka visiems, kurie žaidė, testavo ir metė dar vieną kartą.",
    "Pamoka %d. %s",
    "Taikymo pamoka",
    "Kamuolio masė",
    "Kamuolio sukimas",
    "Alyva ir slydimas",
    "Strike'o linija",
    "MASĖ (%.1fKG)",
    "Lengvo kamuolio testas",
    "Sunkaus kamuolio testas",
    "Išlaikyta",
    "%d / %d pataikymai",
    "MASĖ",
    "SUKIMASIS",
    "SLYDIMAS",
    "SUKIBIMAS",
    "PAPRASTAS",
    "RETAS",
    "EPINIS",
    "LEGENDINIS",
    "Sukimosi testas: %d/%d taikinių (Lygis %d/%d)",
    "Taikymo pamoka: %d/%d taškų",
    "Traukite kamuolį iki galo atgal",
    "Dabar paleiskite",
    "Eikite dešinėn",
    "Eikite kairėn",
    "Alyvos testas: pertepimai %d/%d",
    "Likusi alyva",
    "Pertepimo progresas",
    "Sudėvėkite alyvą prieš pertepdami.",
    "Strike testas: išmuškite STRIKE'ą",
    "1 LYGIS  PIRMASIS ŽINGSNIS",
    "Normalus biomas  Pasiek 100, kad pereitum",
    "2 LYGIS  MALACHAS ATVYKSTA",
    "Normalus biomas  Nugalėk Malachą",
    "3 LYGIS  DYKUMOS ĮSPĖJIMAS",
    "Dykumos biomas  Nugalėk Malachą",
    "4 LYGIS  STIKLINIS LEDAS",
    "Ledo biomas  Nugalėk Malachą",
    "5 LYGIS  NEONO STIKLO KLASĖ",
    "Neono biomas  Nugalėk Malach",
    "6 LYGIS  CHERUBEL NEONE",
    "Neono biomas  Nugalėk Cherubel",
    "7 LYGIS  JĖGOS SMŪGIO KLASĖ",
    "Normalus biomas  Nugalėk Cherubel",
    "8 LYGIS  SMĖLIO MEDIENA",
    "Dykumos biomas  Nugalėk Cherubel",
    "9 LYGIS  SERAPHEL KOPOSE",
    "Dykumos biomas  Nugalėk Seraphel",
    "10 LYGIS  LEDO AUDITORIJA",
    "Ledo biomas  Nugalėk Seraphel",
    "11 LYGIS  PLYTŲ IŠPAŽINTIS",
    "Neono biomas  Nugalėk Seraphel",
    "12 LYGIS  MIESTO RATAI",
    "Neono biomas  Nugalėk Thrones",
    "13 LYGIS  BETONO PARADAS",
    "Normalus biomas  Nugalėk Thrones",
    "Žarijų smūgis",
    "Liepsnos riedėjimas",
    "Ugnies kablys",
    "Pragaro įniršis",
    "Fenikso kilimas",
    "Šalčio slydimas",
    "Vėsos smūgis",
    "Arkties dreifas",
    "Ledyno sukibimas",
    "Pūgos karalius",
    "Žvaigždžių dulkės",
    "Ūko riedėjimas",
    "Kosminis kablys",
    "Tuštumos smūgis",
    "Juodoji skylė",
    "NSO sukinys",
    "Ksenono sukibimas",
    "Miško riedėjimas",
    "Vijoklio smūgis",
    "Griaustinio ąžuolas",
    "Titano šaknis",
    "Pasaulio medis",
    "Šešėlio dreifas",
    "Bedugnės kablys",
    "Anuliatorius",
    "Grandinės riedėjimas",
    "Neono smūgis",
    "Kvantinis kablys",
    "Singuliarumas",
    "Potvynio riedėjimas",
    "Rifo smūgis",
    "Krakeno sukibimas",
    "Leviatanas",
    "Runų kamuolys",
    "Orakulo smūgis",
};

static constexpr const char *g_txl_chars_lt_lt = "MeniuSkčavmEt įoyląKbŽsPrLĮgdGųžAJpDUOIėšzRTV,YBNĄę?.Šf:%ČĖj(c)1/ū\n34$0!–WĘ->wHxCh'2567Ū89ŲF";

static constexpr const char *g_txl_jp_jp[TXL_KEY_COUNT] = {
    "メニュー",
    "チート",
    "学校へ行く",
    "言語",
    "プレイ",
    "練習",
    "フリースタイル",
    "デバイス共有",
    "ゲーム設定",
    "トラッカー",
    "クレジット",
    "ミニゲーム",
    "言語選択",
    "英語",
    "中国語",
    "リトアニア語",
    "日本語",
    "サウンド",
    "曲",
    "オイル",
    "ボール",
    "ショップ",
    "ハイスコア",
    "練習",
    "レーンとボールを選ぶ練習",
    "フリースタイル",
    "解除済みのエンジェル、レーン、ボールで対戦",
    "次のレベル",
    "もう一度",
    "ボーナスゲーム",
    "エネルギー",
    "ボウリングを続けますか？",
    "このボウリングゲームへようこそ。",
    "「READY」をクリックしてゲーム画面にフォーカス。",
    "レディー",
    "エンジェルを選択",
    "対戦相手を選択",
    "レーンを選択",
    "レーンを切り替え",
    "ボールを選択",
    "所持ボール",
    "今すぐ購入",
    "レーン",
    "選択中: %s",
    "レーンを選んでください",
    "レーンがありません",
    "対戦相手のアバター",
    "対戦相手のアバターを選択",
    "対戦相手がいません",
    "所持ボールがありません",
    "ショップは空です",
    "ショップ: ゲームを強化",
    "現在の残高",
    "ロック中",
    "%s にリセット",
    "サウンド設定",
    "品質を変更中...",
    "オーディオモード",
    "SDL バッファ",
    "音楽音量",
    "パフォーマンス低下を検出",
    "オプションを選択してください：",
    "シンセを使用",
    "キャッシュを使用",
    "無効にする",
    "シンセ：リアルタイムOPNチップ合成（プリロードなし、CPU負荷高め）",
    "キャッシュ：事前生成オーディオ（キャッシュ必要、CPU負荷低め）",
    "オーディオをキャッシュ中...",
    "進捗: %d%% (%.1f秒 / %.1f秒)",
    "オイル状態",
    "レーンオイルマップ",
    "トラック情報",
    "最大オイル量: %.1fmm\n現在: %.1fmm\nキャリーダウン: %.3fm/m\n減衰: %.4f/m",
    "滑りやすさ",
    "再オイル: 無料",
    "再オイル費用: $%.0f (所持金: $%.0f)",
    "再オイル: %d/%d",
    "再オイル",
    "資金不足",
    "再オイルはロックされています",
    "オイル済み",
    "トップスコア",
    "あなたのスコアはトップ%dに入っています",
    "良いゲーム！",
    "%dポイント",
    "あなたのスコアは最近の全プレイの%.0f%%を上回っています",
    "リーダーボード（直近1時間）",
    "プレイヤー",
    "スコア",
    "時間",
    "まだスコアがありません - 最初に記録しよう！",
    "試行回数: %d",
    "範囲: %d～%d",
    "ゲーム設定",
    "雪片: %d / %d",
    "雪の密度",
    "雪は完全に無効（更新・描画を含む）",
    "進行状況をリセット",
    "キャンペーンレベル、アンロック、所持金をリセット。ユーザー名はそのまま。",
    "更新を確認",
    "PWA を更新",
    "ウェブサイトを再読み込み",
    "インストール済みビルド: %s",
    "公開ビルド: %s",
    "更新を確認中...",
    "最新です。公開ビルド: %s",
    "更新があります: %s -> %s",
    "オフラインです。ネット接続後に再確認してください。",
    "更新サーバーに接続できませんでした。",
    "更新を適用しています...",
    "PWA 更新確認はウェブ版でのみ利用できます。",
    "ゲームデザイン、プログラム、アート、サウンド: %s。",
    "Lape Kale",
    "キャラクター基礎モデル: OpenGameArt.org の Very Low Poly Human。",
    "キャラクターアニメーション: Mixamo。",
    "SDL、OpenGL ES、Clay、GLM、Jolt Physics、ymfm を使用。",
    "プレイ、テスト、そしてもう1フレーム投げ続けてくれた皆さんに感謝します。",
    "レッスン %d. %s",
    "エイムレッスン",
    "ボールの質量",
    "ボールに回転をかける",
    "オイルとスキッド",
    "ストライクライン",
    "質量 (%.1fkg)",
    "軽いボールのテスト",
    "重いボールのテスト",
    "合格",
    "%d / %d ヒット",
    "質量",
    "回転",
    "スキッド",
    "バイト",
    "コモン",
    "レア",
    "エピック",
    "レジェンダリー",
    "スピンテスト: %d/%d ターゲット (レベル %d/%d)",
    "エイムレッスン: %d/%d ポイント",
    "ボールを一番後ろまで引いてください",
    "今すぐ放してください",
    "右に移動",
    "左に移動",
    "オイルテスト: 再オイル %d/%d",
    "残りのオイル",
    "再オイルの進行状況",
    "再オイルする前に使い切ってください。",
    "ストライクテスト: ストライクを取れ",
    "レベル1 最初の目標",
    "ノーマルバイオーム 100点でクリア",
    "レベル2 マラック登場",
    "ノーマルバイオーム マラックに勝利",
    "レベル3 砂漠の警告",
    "砂漠バイオーム マラックに勝利",
    "レベル4 ガラスの氷",
    "氷バイオーム マラックに勝利",
    "レベル5 ネオン硝子",
    "ネオン  Malachに勝利",
    "レベル6 Cherubel",
    "ネオン  Cherubelに勝利",
    "レベル7 パワー教室",
    "ノーマル  Cherubelに勝利",
    "レベル8 砂の材木",
    "砂漠  Cherubelに勝利",
    "レベル9 砂丘のSeraphel",
    "砂漠  Seraphelに勝利",
    "レベル10 氷の観客",
    "氷  Seraphelに勝利",
    "レベル11 レンガ告白",
    "ネオン  Seraphelに勝利",
    "レベル12 街の車輪",
    "ネオン  Thronesに勝利",
    "レベル13 コンクリート",
    "ノーマル  Thronesに勝利",
    "残り火ストライク",
    "炎ロール",
    "ブレイズフック",
    "インフェルノ",
    "フェニックス",
    "フロストグライド",
    "チルストライク",
    "北極ドリフト",
    "氷河バイト",
    "吹雪キング",
    "星くず",
    "星雲ロール",
    "宇宙フック",
    "虚空ストライク",
    "ブラックホール",
    "UFOスピン",
    "キセノンバイト",
    "森ロール",
    "ツタストライク",
    "雷オーク",
    "巨人の根",
    "世界樹",
    "影ドリフト",
    "深淵フック",
    "ヌリファイア",
    "回路ロール",
    "ネオンストライク",
    "量子フック",
    "特異点",
    "潮ロール",
    "リーフストライク",
    "クラーケンバイト",
    "リヴァイアサン",
    "ルーンボール",
    "オラクルストライク",
};

static constexpr const char *g_txl_chars_jp_jp = "メニューチト学校へ行く言語プレイ練習フリスタルデバ共有ゲム設定ラッカクジミ選択英中国ア日本サウンド曲オボショハコとをぶ解除済みのエェ、で対戦次ベもう一度ナネギグ続けますか？こよそ。「READY」して画面にォィ相手切り替え所持今ぐ購入: %sんださいがあせは空強化現在残高ロセ品質変更.モSLァ音楽量パマ低下検出：使用キャ無効るOPN合成（なCU負荷め）事前生必要進捗d(1f秒/)状態情報最大m\nダ3減衰4滑や再料費$0金資不足れたっ良！ポ近全上回直時間ヤ-初記録試数範囲～雪片密完新・描含む況ペユザ名確認Wブ読込ビ公開>接後き適版利apeKl基礎nGrtogVywHuMixJhcテ投げ皆感謝転k軽重格ヒピ番ろ引放右移動左取目標ノ点2登場勝砂漠警告ガ氷5硝子6b7ワ教室8材木9丘観客白街車輪T火炎ズ北極河吹星ず雲宇宙虚ホF森ツ雷巨人根世界樹影深淵ヌ路特異潮ケヴė";

static constexpr const char *g_txl_zh_cn[TXL_KEY_COUNT] = {
    "菜单",
    "作弊",
    "去学校",
    "语言",
    "开始",
    "练习",
    "自由赛",
    "设备共享",
    "游戏设置",
    "音轨编辑",
    "制作名单",
    "小游戏",
    "选择语言",
    "English",
    "中文",
    "立陶宛语",
    "日语",
    "声音",
    "歌曲",
    "油道",
    "球库",
    "商店",
    "最高分",
    "练习",
    "选择球道和球的练习局",
    "自由赛",
    "使用已解锁的天使、球道和球对战",
    "下一关",
    "再试一次",
    "奖励关",
    "能量",
    "你想继续打保龄球吗？",
    "欢迎来到这款保龄球游戏。",
    "点击“准备”以聚焦游戏画面。",
    "准备",
    "选择天使",
    "选择对手",
    "选择球道",
    "切换球道",
    "选择球",
    "库存",
    "立即购买",
    "球道",
    "已选择：%s",
    "选择一个球道",
    "没有球道",
    "对手形象",
    "选择一个对手形象",
    "没有对手",
    "库存为空",
    "商店已空",
    "商店：强化这一局",
    "当前余额",
    "未解锁",
    "%s后重置",
    "声音设置",
    "正在切换音频模式...",
    "音频模式",
    "SDL 缓冲",
    "音乐音量",
    "检测到性能偏低",
    "请选择一个选项：",
    "使用合成",
    "使用缓存",
    "关闭音频",
    "合成：实时 OPN 芯片合成（无需预载，CPU 占用更高）",
    "缓存：预生成音频数据（需要缓存，CPU 占用更低）",
    "正在缓存音频...",
    "进度：%d%%（%.1f秒 / %.1f秒）",
    "油道状态",
    "油图",
    "球道信息",
    "最大油量：%.1f毫米\n当前：%.1f毫米\n带油：%.3f米/米\n衰减：%.4f/米",
    "滑度",
    "重新上油：免费",
    "重新上油费用：$%.0f（你有：$%.0f）",
    "重新上油：%d/%d",
    "重新上油",
    "余额不足",
    "暂时不能上油",
    "刚上过油",
    "最高分",
    "你的分数进入前%d名",
    "不错的一局！",
    "%d分",
    "你的分数超过了近期%.0f%%的对局",
    "排行榜（最近一小时）",
    "玩家",
    "分数",
    "时间",
    "还没有分数，来当第一个吧！",
    "尝试次数：%d",
    "范围：%d-%d",
    "游戏设置",
    "雪花：%d / %d",
    "雪花密度",
    "雪已完全关闭，包括更新和绘制。",
    "重置进度",
    "重置战役关卡、解锁内容和现金。用户名保持不变。",
    "检查更新",
    "更新 PWA",
    "重新加载网站",
    "已安装构建：%s",
    "已发布构建：%s",
    "正在检查更新……",
    "已是最新。已发布构建：%s",
    "有可用更新：%s -> %s",
    "当前离线。请连接网络后检查更新。",
    "无法连接更新服务器。",
    "正在应用更新……",
    "只有网页应用支持 PWA 更新检查。",
    "游戏设计、程序、美术和声音：%s。",
    "Lape Kale",
    "角色基础模型：OpenGameArt.org 的 Very Low Poly Human。",
    "角色动画来自 Mixamo。",
    "使用 SDL、OpenGL ES、Clay、GLM、Jolt Physics 和 ymfm 构建。",
    "特别感谢每一位游玩、测试，并继续多投一局的人。",
    "第%d课：%s",
    "瞄准课",
    "球的质量",
    "给球上旋",
    "油与滑行",
    "全中线路",
    "质量（%.1f公斤）",
    "轻球测试",
    "重球测试",
    "已通过",
    "%d / %d 次击中",
    "质量",
    "旋转",
    "滑行",
    "咬道",
    "普通",
    "稀有",
    "史诗",
    "传说",
    "旋转测试：%d/%d 目标（第%d/%d关）",
    "瞄准课：%d/%d分",
    "把球尽量向后拉",
    "现在放手",
    "向右移动",
    "向左移动",
    "油道测试：重新上油 %d/%d",
    "剩余油量",
    "上油进度",
    "先把油耗掉，再重新上油。",
    "全中测试：打出STRIKE",
    "第1关  第一个里程碑",
    "普通球道  达到100分即可通过",
    "第2关  玛拉克到来",
    "普通球道  击败玛拉克",
    "第3关  沙漠警告",
    "沙漠球道  击败玛拉克",
    "第4关  玻璃冰面",
    "冰面球道  击败玛拉克",
    "第5关  霓虹玻璃课",
    "霓虹球道  击败Malach",
    "第6关  Cherubel在霓虹",
    "霓虹球道  击败Cherubel",
    "第7关  强力球课",
    "普通球道  击败Cherubel",
    "第8关  沙地木障",
    "沙漠球道  击败Cherubel",
    "第9关  沙丘中的Seraphel",
    "沙漠球道  击败Seraphel",
    "第10关  冰上会面",
    "冰面球道  击败Seraphel",
    "第11关  砖块告白",
    "霓虹球道  击败Seraphel",
    "第12关  城市之轮",
    "霓虹球道  击败Thrones",
    "第13关  混凝土游行",
    "普通球道  击败Thrones",
    "余烬强击",
    "烈焰滚",
    "火焰钩",
    "炼狱怒",
    "凤凰升",
    "霜滑",
    "寒意强击",
    "极地漂移",
    "冰川咬道",
    "暴雪王",
    "星尘",
    "星云滚",
    "宇宙钩",
    "虚空强击",
    "黑洞",
    "UFO旋",
    "氙光咬道",
    "森林滚",
    "藤蔓强击",
    "雷橡",
    "泰坦根",
    "世界树",
    "暗影漂移",
    "深渊钩",
    "归零者",
    "电路滚",
    "霓虹强击",
    "量子钩",
    "奇点",
    "潮汐滚",
    "礁石强击",
    "海妖咬道",
    "利维坦",
    "符文球",
    "神谕强击",
};

static constexpr const char *g_txl_chars_zh_cn = "菜单作弊去学校语言开始练习自由赛设备共享游戏置音轨编辑制名小选择English中文立陶宛日声歌曲油道球库商店最高分和的局使用已解锁天、对战下一关再试次奖励能量你想继续打保龄吗？欢迎来到这款。点击“准”以聚焦画面手切换存即购买：%个没有形象为空强化当前余额未后重正在频模式.SDL 缓冲乐检测性偏低请项合成闭实时OPN芯片（无需预载，CU占更）生数据要进度d1f秒/状态图信息大毫米\n带3衰减4滑新上免费$0不足暂刚过入错！超了近期排行榜玩家间还第吧尝范围-雪花密完全包括绘役卡内容现金户持变查WA加网站安装构建发布…是可>离线连接络法服务器应只页支计程序美术apeK角色基础型GmrtoVywHu动MxJc特别感谢每位并多投人课瞄质给旋与路公斤轻通转咬普稀史诗传说目标把尽向拉放右移左剩先耗掉出TRI里碑达2玛克败沙漠警告玻璃冰5霓虹6b7力8地木障9丘会砖块白城市之轮混凝土烬烈焰滚火钩炼狱怒凤凰升霜寒意极漂川暴王星尘云宇宙虚黑洞F氙光森林藤蔓雷橡泰坦根世界树暗影深渊归零者电子奇潮汐礁石海妖利维符神谕ė";

