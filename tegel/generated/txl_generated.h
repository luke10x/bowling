#pragma once

// Generated by scripts/generate_txl.py

enum TxlKey
{
    TXL_MENU,
    TXL_RENAME,
    TXL_GO_TO_SCHOOL,
    TXL_LANGUAGE,
    TXL_CAMPAIGN,
    TXL_PRACTICE,
    TXL_FREESTYLE,
    TXL_DEVICE_SHARE,
    TXL_GAME_SETTINGS,
    TXL_TRACKER,
    TXL_CREDITS,
    TXL_MINIGAMES,
    TXL_SELECT_LANGUAGE,
    TXL_LANGUAGE_ENGLISH,
    TXL_LANGUAGE_CHINESE,
    TXL_LANGUAGE_LITHUANIAN,
    TXL_LANGUAGE_JAPANESE,
    TXL_SOUND,
    TXL_SONG,
    TXL_OIL,
    TXL_BALLS,
    TXL_SHOP,
    TXL_HI_SCORE,
    TXL_PRACTICE_TITLE,
    TXL_PRACTICE_SUBTITLE,
    TXL_FREESTYLE_TITLE,
    TXL_FREESTYLE_SUBTITLE,
    TXL_NEXT_LEVEL,
    TXL_TRY_AGAIN,
    TXL_BONUS_GAME,
    TXL_BALL_CHARGE_FMT,
    TXL_GREETING_RESUME,
    TXL_GREETING_WELCOME,
    TXL_GREETING_SUBLINE,
    TXL_READY,
    TXL_SELECT_ANGEL,
    TXL_SELECT_BOT,
    TXL_SELECT_HOUSE,
    TXL_SWITCH_HOUSE,
    TXL_SELECT_BALL,
    TXL_INVENTORY,
    TXL_BUY_NOW,
    TXL_HOUSES,
    TXL_SELECTED_FMT,
    TXL_SELECT_A_HOUSE,
    TXL_NO_HOUSES,
    TXL_BOT_AVATAR,
    TXL_SELECT_A_BOT_AVATAR,
    TXL_NO_BOTS,
    TXL_INVENTORY_EMPTY,
    TXL_SHOP_EMPTY,
    TXL_SHOP_IMPROVE_YOUR_RUN,
    TXL_CURRENT_BALANCE,
    TXL_LOCKED,
    TXL_RESETS_IN_FMT,
    TXL_SOUND_SETTINGS,
    TXL_CHANGING_QUALITY,
    TXL_AUDIO_MODE,
    TXL_SDL_BUFFER,
    TXL_MUSIC_VOLUME,
    TXL_LOW_PERFORMANCE_DETECTED,
    TXL_PLEASE_CHOOSE_OPTION,
    TXL_USE_SYNTH,
    TXL_USE_CACHED,
    TXL_DISABLE,
    TXL_SYNTH_EXPLAIN,
    TXL_CACHED_EXPLAIN,
    TXL_CACHING_AUDIO,
    TXL_PROGRESS_FMT,
    TXL_OIL_STATUS,
    TXL_LANE_OIL_MAP,
    TXL_TRACK_INFO,
    TXL_MAX_OIL_LEVEL_FMT,
    TXL_SLIPPERY,
    TXL_REOIL_FREE,
    TXL_REOIL_COST_FMT,
    TXL_REOILS_FMT,
    TXL_REOIL,
    TXL_CANT_AFFORD,
    TXL_REOIL_LOCKED,
    TXL_FRESHLY_OILED,
    TXL_TOP_SCORES,
    TXL_YOUR_SCORE_TOP_FMT,
    TXL_GOOD_RUN,
    TXL_POINTS_FMT,
    TXL_SCORE_HIGHER_THAN_FMT,
    TXL_LEADERBOARD_LAST_HOUR,
    TXL_PLAYER,
    TXL_SCORE,
    TXL_AGE,
    TXL_NO_SCORES_YET,
    TXL_ATTEMPTS_FMT,
    TXL_RANGE_FMT,
    TXL_GAME_SETTINGS_TITLE,
    TXL_SNOWFLAKES_FMT,
    TXL_SNOWFLAKE_DENSITY,
    TXL_SNOW_DISABLED,
    TXL_RESET_PROGRESS,
    TXL_RESET_PROGRESS_HELP,
    TXL_CHECK_FOR_UPDATE,
    TXL_UPDATE_PWA,
    TXL_UPDATE_WEB,
    TXL_PWA_UPDATE_INSTALLED_FMT,
    TXL_PWA_UPDATE_PUBLISHED_FMT,
    TXL_PWA_UPDATE_CHECKING,
    TXL_PWA_UPDATE_UP_TO_DATE_FMT,
    TXL_PWA_UPDATE_AVAILABLE_FMT,
    TXL_PWA_UPDATE_OFFLINE,
    TXL_PWA_UPDATE_ERROR,
    TXL_PWA_UPDATE_APPLYING,
    TXL_PWA_UPDATE_UNSUPPORTED,
    TXL_CREDITS_AUTHOR_FMT,
    TXL_CREDITS_AUTHOR_NAME,
    TXL_CREDITS_CHARACTER_BASE,
    TXL_CREDITS_MIXAMO,
    TXL_CREDITS_TECH,
    TXL_CREDITS_THANKS,
    TXL_SCHOOL_TITLE_FMT,
    TXL_AIM_LESSON,
    TXL_BALL_MASS,
    TXL_SPIN_BALL,
    TXL_OIL_AND_SKID,
    TXL_STRIKE_LINE,
    TXL_MASS_FMT,
    TXL_LIGHT_BALL_TEST,
    TXL_HEAVY_BALL_TEST,
    TXL_PASSED,
    TXL_HITS_FMT,
    TXL_MASS,
    TXL_SPIN,
    TXL_SKID,
    TXL_BITE,
    TXL_RARITY_COMMON,
    TXL_RARITY_RARE,
    TXL_RARITY_EPIC,
    TXL_RARITY_LEGENDARY,
    TXL_SPIN_TEST_FMT,
    TXL_AIM_LESSON_POINTS_FMT,
    TXL_PULL_BALL_BACK,
    TXL_LET_IT_GO_NOW,
    TXL_MOVE_RIGHT,
    TXL_MOVE_LEFT,
    TXL_OIL_TEST_REOILS_FMT,
    TXL_REMAINING_OIL,
    TXL_REOIL_PROGRESS,
    TXL_WEAR_IT_DOWN,
    TXL_STRIKE_TEST_SCORE_STRIKE,
    TXL_LEVEL1_TITLE,
    TXL_LEVEL1_SUBTITLE,
    TXL_LEVEL2_TITLE,
    TXL_LEVEL2_SUBTITLE,
    TXL_LEVEL3_TITLE,
    TXL_LEVEL3_SUBTITLE,
    TXL_LEVEL4_TITLE,
    TXL_LEVEL4_SUBTITLE,
    TXL_LEVEL5_TITLE,
    TXL_LEVEL5_SUBTITLE,
    TXL_LEVEL6_TITLE,
    TXL_LEVEL6_SUBTITLE,
    TXL_LEVEL7_TITLE,
    TXL_LEVEL7_SUBTITLE,
    TXL_LEVEL8_TITLE,
    TXL_LEVEL8_SUBTITLE,
    TXL_LEVEL9_TITLE,
    TXL_LEVEL9_SUBTITLE,
    TXL_LEVEL10_TITLE,
    TXL_LEVEL10_SUBTITLE,
    TXL_LEVEL11_TITLE,
    TXL_LEVEL11_SUBTITLE,
    TXL_LEVEL12_TITLE,
    TXL_LEVEL12_SUBTITLE,
    TXL_LEVEL13_TITLE,
    TXL_LEVEL13_SUBTITLE,
    TXL_BALL_NAME_0,
    TXL_BALL_NAME_1,
    TXL_BALL_NAME_2,
    TXL_BALL_NAME_3,
    TXL_BALL_NAME_4,
    TXL_BALL_NAME_5,
    TXL_BALL_NAME_6,
    TXL_BALL_NAME_7,
    TXL_BALL_NAME_8,
    TXL_BALL_NAME_9,
    TXL_BALL_NAME_10,
    TXL_BALL_NAME_11,
    TXL_BALL_NAME_12,
    TXL_BALL_NAME_13,
    TXL_BALL_NAME_14,
    TXL_BALL_NAME_15,
    TXL_BALL_NAME_16,
    TXL_BALL_NAME_17,
    TXL_BALL_NAME_18,
    TXL_BALL_NAME_19,
    TXL_BALL_NAME_20,
    TXL_BALL_NAME_21,
    TXL_BALL_NAME_22,
    TXL_BALL_NAME_23,
    TXL_BALL_NAME_24,
    TXL_BALL_NAME_25,
    TXL_BALL_NAME_26,
    TXL_BALL_NAME_27,
    TXL_BALL_NAME_28,
    TXL_BALL_NAME_29,
    TXL_BALL_NAME_30,
    TXL_BALL_NAME_31,
    TXL_BALL_NAME_32,
    TXL_BALL_NAME_33,
    TXL_BALL_NAME_34,
    TXL_YOU,
    TXL_BONUS_ROUND,
    TXL_TUTORIAL_FMT,
    TXL_COMPLETION_TIME_FMT,
    TXL_AUDIO_BUFFER_INCREASED,
    TXL_SPIN_TO_HOOK,
    TXL_DROP_HERE_TO_USE,
    TXL_CANNOT_USE_NOW,
    TXL_NEED_AT_LEAST_2_BALLS,
    TXL_RUNE_OUTCOME_DEFAULT,
    TXL_RUNE_OUTCOME_BALL_LOST,
    TXL_RUNE_OUTCOME_BALL_EVAPORATED,
    TXL_RUNE_OUTCOME_BALL_SURVIVED,
    TXL_RUNE_OUTCOME_PINS_FROZEN,
    TXL_RUNE_OUTCOME_PATROL_DEPLOYED,
    TXL_RUNE_OUTCOME_FOOTBALL,
    TXL_RUNE_OUTCOME_FLASH,
    TXL_RUNE_OUTCOME_SKULL,
    TXL_RUNE_OUTCOME_ICY_BALL,
    TXL_RUNE_OUTCOME_BOOM,
    TXL_MISSED,
    TXL_STALLED,
    TXL_SPLIT,
    TXL_STRIKE,
    TXL_SPARE,
    TXL_BLOCK_WOOD,
    TXL_BLOCK_BRICK,
    TXL_BLOCK_CONCRETE,
    TXL_BLOCK_GLASS,
    TXL_CHEST_TITLE,
    TXL_TAP_TO_OPEN_CHEST,
    TXL_CHEST_RUNE_PICKED_FMT,
    TXL_CHEST_COINS_PICKED_FMT,
    TXL_CONTINUE,
    TXL_RUNE_NAME_BOOM,
    TXL_RUNE_NAME_BOLT,
    TXL_RUNE_NAME_FREEZE,
    TXL_RUNE_NAME_SKULL,
    TXL_RUNE_NAME_GUARD_PINS,
    TXL_RUNE_NAME_FOOTBALL,
    TXL_RUNE_NAME_DEFAULT,
    TXL_RUNE_DESC_BOOM,
    TXL_RUNE_DESC_BOLT,
    TXL_RUNE_DESC_FREEZE,
    TXL_RUNE_DESC_SKULL,
    TXL_RUNE_DESC_GUARD_PINS,
    TXL_RUNE_DESC_FOOTBALL,
    TXL_RUNE_DESC_DEFAULT,
    TXL_MINIGAMES_HELP,
    TXL_MINIGAME_COIN_RUSH,
    TXL_MINIGAME_COUNT_MASTERS,
    TXL_MINIGAME_CROWD_CONTROL,
    TXL_BONUS_PROMPT_FMT,
    TXL_PRIZE_BALL_FMT,
    TXL_RESET_PROGRESS_CONFIRM_TITLE,
    TXL_ARE_YOU_SURE,
    TXL_RESET_PROGRESS_FULL_DETAIL,
    TXL_RESET_PROGRESS_SOFT_DETAIL,
    TXL_NO,
    TXL_YES,
    TXL_EXIT_BONUS_LEVEL,
    TXL_EXIT_BONUS_LEVEL_CONFIRM,
    TXL_STRIKES,
    TXL_SPARES,
    TXL_COINS,
    TXL_WON_BY_POINTS,
    TXL_TOTAL,
    TXL_COINS_OBTAINED,
    TXL_VICTORY,
    TXL_YOU_LOSE,
    TXL_NEXT,
    TXL_RETRY,
    TXL_REPEAT,
    TXL_INVENTORY_SHORT,
    TXL_RELOADS_IN_FMT,
    TXL_RESTOCK_SOON,
    TXL_BONUS_LEVEL,
    TXL_PLAY_BONUS,
    TXL_BONUS_ROUND_COMPLETE,
    TXL_COINS_PICKED_EARNED_FMT,
    TXL_BONUS_COMPLETE_FMT,
    TXL_EXITED_EARNED_FMT,
    TXL_BONUS_EXITED_FMT,
    TXL_BONUS_ROUND_VICTORY,
    TXL_BONUS_ROUND_DEFEAT,
    TXL_BONUS_ROUND_LOST,
    TXL_CROWD_VICTORY_BASE,
    TXL_CROWD_DEFEAT_SPAWN,
    TXL_VICTORY_DETAIL,
    TXL_DEFEAT_DETAIL,
    TXL_BALL_REWARD_FMT,
    TXL_ENEMIES,
    TXL_BOSSES,
    TXL_SUPER_BOSS,
    TXL_GATES,
    TXL_PINS,
    TXL_STANDERS,
    TXL_YOU_BEAT_ANGEL,
    TXL_ANGEL_WINS,
    TXL_BOT_SCORE_FMT,
    TXL_CAMPAIGN_COMPLETE,
    TXL_CAMPAIGN_COMPLETE_DETAIL,
    TXL_TOTAL_TIME_FMT,
    TXL_ATTEMPTS_TOTAL_FMT,
    TXL_ATTEMPTS_BY_LEVEL,
    TXL_LEVEL_ATTEMPTS_FMT,
    TXL_ENTER_CHEAT,
    TXL_CHEAT_ACTIVATED,
    TXL_NO_SUCH_CHEAT,
    TXL_MAKES_YOU_SPAWN_FAST,
    TXL_MAKES_YOU_STRONG,
    TXL_TURN_FMT,
    TXL_NOS,
    TXL_BONUS_AVAILABLE,
    TXL_SHOP_UPDATED,
    TXL_SHOP_RESTOCK_PROMPT,
    TXL_VISIT_SHOP,
    TXL_LATER,
    TXL_KEYPAD_ENTER_TEXT,
    TXL_KEYPAD_ENTER_NUMBER,
    TXL_DELETE,
    TXL_ENTER,
    TXL_STRESS,
    TXL_KEY_COUNT
};

static constexpr const char *g_txl_en_us[TXL_KEY_COUNT] = {
    "Menu",
    "Cheats",
    "Go to school",
    "Language",
    "Play",
    "Practice",
    "Freestyle",
    "Device Share",
    "Game Settings",
    "Tracker",
    "Credits",
    "MINIGAMES",
    "Select Language",
    "English",
    "Chinese",
    "Lithuanian",
    "Japanese",
    "Sound",
    "Song",
    "Oil",
    "BALLS",
    "SHOP",
    "Hi-Score",
    "PRACTICE",
    "House and ball selection run",
    "FREESTYLE",
    "Versus with your unlocked angel, house, and ball",
    "NEXT LEVEL",
    "TRY AGAIN",
    "BONUS GAME",
    "ENERGY",
    "Do you want to continue to play bowling?",
    "Welcome to this bowling game.",
    "Click READY to focus the game canvas.",
    "READY",
    "SELECT ANGEL",
    "SELECT BOT",
    "SELECT HOUSE",
    "SWITCH HOUSE",
    "SELECT BALL",
    "INVENTORY",
    "BUY NOW",
    "Houses",
    "Selected: %s",
    "Select a house",
    "No houses",
    "Bot Avatar",
    "Select a bot avatar",
    "No bots",
    "INVENTORY IS EMPTY",
    "SHOP IS EMPTY",
    "SHOP: IMPROVE YOUR RUN",
    "Current balance",
    "LOCKED",
    "Resets in %s",
    "Sound Settings",
    "Changing quality...",
    "Audio Mode",
    "SDL Buffer",
    "Music Volume",
    "Low Performance Detected",
    "Please choose an option:",
    "Use Synth",
    "Use Cached",
    "Disable",
    "Synth: Real-time OPN chip synthesis (no preload, more CPU)",
    "Cached: Pre-generated audio blobs (needs caching, lighter on CPU)",
    "Caching Audio...",
    "Progress: %d%% (%.1fs / %.1fs)",
    "Oil Status",
    "Lane Oil Map",
    "Track Info",
    "Max oil level: %.1fmm\nCurrent: %.1fmm\nCarrydown: %.3fm/m\nDecay: %.4f/m",
    "Slippery",
    "Re-oil: FREE",
    "Re-oil cost: $%.0f  (you have: $%.0f)",
    "Re-oils: %d/%d",
    "RE-OIL",
    "CAN'T AFFORD",
    "RE-OIL LOCKED",
    "FRESHLY OILED",
    "Top Scores",
    "Your score is in top %d",
    "Good Run!",
    "%d points",
    "Your score is higher than %.0f%% of all recent runs",
    "Leaderboard (Last Hour)",
    "Player",
    "Score",
    "Age",
    "No scores yet - be the first!",
    "Attempts: %d",
    "Range: %d-%d",
    "Game Settings",
    "Snowflakes: %d / %d",
    "Snowflake density",
    "Snow is fully disabled, including updates and draw calls.",
    "RESET PROGRESS",
    "Resets campaign level, unlocks, and cash. Username stays unchanged.",
    "CHECK FOR UPDATE",
    "UPDATE PWA",
    "RELOAD WEBSITE",
    "Installed build: %s",
    "Published build: %s",
    "Checking for update...",
    "Up to date. Published build: %s",
    "Update available: %s -> %s",
    "Offline. Connect to the internet to check for updates.",
    "Could not reach the update server.",
    "Applying update...",
    "PWA update checks are only available in the web app.",
    "Game design, programming, art, and sound by %s.",
    "Lapė Kalė",
    "Character base: Very Low Poly Human from OpenGameArt.org.",
    "Character animations from Mixamo.",
    "Built with SDL, OpenGL ES, Clay, GLM, Jolt Physics, and ymfm.",
    "Special thanks to everyone who played, tested, and kept throwing another frame.",
    "Lesson %d. %s",
    "Aim lesson",
    "Ball Mass",
    "Spin ball",
    "Oil and skid",
    "Strike line",
    "MASS (%.1fKG)",
    "Light ball test",
    "Heavy ball test",
    "Passed",
    "%d / %d hits",
    "MASS",
    "SPIN",
    "SKID",
    "BITE",
    "COMMON",
    "RARE",
    "EPIC",
    "LEGENDARY",
    "Spin test: %d/%d targets (Level %d/%d)",
    "Aim lesson: %d/%d points",
    "Pull the ball all the way back",
    "Let it go now",
    "Move right",
    "Move left",
    "Oil test: re-oils %d/%d",
    "Remaining oil",
    "Re-oil progress",
    "Wear it down before re-oiling.",
    "Strike test: score a STRIKE",
    "LEVEL 1  FIRST MILESTONE",
    "Normal biome  Reach 100 to pass",
    "LEVEL 2  MALACH ARRIVES",
    "Normal biome  Beat Malach",
    "LEVEL 3  DESERT WARNING",
    "Desert biome  Beat Malach",
    "LEVEL 4  GLASS ICE",
    "Ice biome  Beat Malach",
    "LEVEL 5  NEON GLASS CLASS",
    "Neon biome  Beat Malach",
    "LEVEL 6  CHERUBEL IN NEON",
    "Neon biome  Beat Cherubel",
    "LEVEL 7  POWER SHOT CLASS",
    "Normal biome  Beat Cherubel",
    "LEVEL 8  SAND TIMBER",
    "Desert biome  Beat Cherubel",
    "LEVEL 9  SERAPHEL IN THE DUNES",
    "Desert biome  Beat Seraphel",
    "LEVEL 10  ICE AUDIENCE",
    "Ice biome  Beat Seraphel",
    "LEVEL 11  BRICK CONFESSION",
    "Neon biome  Beat Seraphel",
    "LEVEL 12  WHEELS OF THE CITY",
    "Neon biome  Beat Thrones",
    "LEVEL 13  CONCRETE PARADE",
    "Normal biome  Beat Thrones",
    "Ember Strike",
    "Flame Roll",
    "Blaze Hook",
    "Inferno Fury",
    "Phoenix Rise",
    "Frost Glide",
    "Chill Strike",
    "Arctic Drift",
    "Glacier Bite",
    "Blizzard King",
    "Star Dust",
    "Nebula Roll",
    "Cosmic Hook",
    "Void Strike",
    "Black Hole",
    "UFO Spin",
    "Xenon Bite",
    "Forest Roll",
    "Vine Strike",
    "Thunder Oak",
    "Titan Root",
    "World Tree",
    "Shadow Drift",
    "Abyss Hook",
    "Nullifier",
    "Circuit Roll",
    "Neon Strike",
    "Quantum Hook",
    "Singularity",
    "Tide Roll",
    "Reef Strike",
    "Kraken Bite",
    "Leviathan",
    "Rune Ball",
    "Oracle Strike",
    "you",
    "BONUS ROUND",
    "LESSON %d",
    "COMPLETION TIME %s",
    "Audio buffer increased to 4096 due to performance",
    "spin to hook",
    "DROP HERE\nTO USE",
    "CANNOT USE\nIT NOW",
    "NEED AT LEAST\n2 BALLS",
    "RUNE",
    "BALL LOST IN EXPLOSION",
    "BALL EVAPORATED",
    "BALL SURVIVED THE FLASH",
    "PINS FROZEN",
    "PATROL PINS DEPLOYED",
    "FOOTBALL ACTIVATED",
    "FLASH ACTIVATED",
    "SKULL ACTIVATED",
    "ICY BALL ACTIVATED",
    "BOOM ACTIVATED",
    "MISSED",
    "STALLED",
    "SPLIT",
    "STRIKE",
    "SPARE",
    "WOOD",
    "BRICK",
    "CONCRETE",
    "GLASS",
    "Treasure Chest",
    "TAP TO OPEN CHEST",
    "You picked up this ability: %s",
    "You picked up %d coins",
    "Continue",
    "Boom Rune",
    "Bolt Rune",
    "Freeze Rune",
    "Skull Rune",
    "Patrol Pins Rune",
    "Football Rune",
    "Rune",
    "Explosive shot. Strong impact, risky ball loss.",
    "Lightning defense: evaporates the enemy ball.",
    "Freezes pins on defense or makes your ball icy.",
    "Skull ball: harder hits and stronger block damage.",
    "Deploys three patrol pins against the enemy.",
    "Bounces the active ball and makes it faster.",
    "Adds an ability to your rune tray.",
    "Testing access. Rewards count; story does not.",
    "COIN RUSH",
    "COUNT MASTERS",
    "CROWD CONTROL",
    "Do you want to play %s?%s",
    " Prize ball: %s",
    "Reset Progress",
    "Are you sure?",
    "This erases campaign progress, school completion, coins, balls, and unlocks.",
    "This restarts the campaign but keeps your ball inventory.",
    "No",
    "Yes",
    "Exit Bonus Level",
    "Exit this bonus level in progress?",
    "Strikes",
    "Spares",
    "Coins",
    "Won by points",
    "Total",
    "COINS OBTAINED",
    "VICTORY",
    "YOU LOSE",
    "NEXT",
    "RETRY",
    "REPEAT",
    "INVENTORY",
    "Reloads in %s",
    "RESTOCK SOON",
    "BONUS LEVEL",
    "PLAY BONUS",
    "BONUS ROUND COMPLETE",
    "COINS PICKED: %d   EARNED: $%d",
    "BONUS COMPLETE  +$%d",
    "Exited before completion.   EARNED SO FAR: $%d",
    "BONUS ROUND EXITED  +$%d",
    "BONUS ROUND VICTORY",
    "BONUS ROUND DEFEAT",
    "BONUS ROUND LOST",
    "Victory: malachim reached the enemy base.",
    "Defeat: enemies reached your spawn point.",
    "Victory.",
    "Defeat.",
    "   BALL: %s",
    "Enemies",
    "Bosses",
    "Super-boss",
    "Gates",
    "Pins",
    "Standers",
    "YOU BEAT ANGEL",
    "ANGEL WINS",
    "You: %d    Angel: %d",
    "CAMPAIGN COMPLETE",
    "Every level cleared. The lane is yours.",
    "Total Time  %s",
    "Attempts  %d",
    "Attempts by level",
    "L%d  %d",
    "Enter Cheat",
    "cheat activated",
    "no such cheat",
    "MAKES YOU SPAWN FAST",
    "MAKES YOU STRONG",
    "%s TURN",
    "NOS",
    "BONUS AVAILABLE",
    "SHOP UPDATED",
    "New balls are in stock. Do you want to visit the shop before the next game?",
    "VISIT SHOP",
    "LATER",
    "Enter Text",
    "Enter Number",
    "Delete",
    "Enter",
    "Stress",
};

static constexpr const char *g_txl_chars_en_us = "MenuChatsGo clLgPyriFDvSmTkdINAEJpOBH-RbYVw,XU?W.f:%Kq()1/x\n34$0'!>ė256789zQZ;+";

static constexpr const char *g_txl_lt_lt[TXL_KEY_COUNT] = {
    "Meniu",
    "Sukčiavimai",
    "Eiti į mokyklą",
    "Kalba",
    "Žaisti",
    "Praktika",
    "Laisvas stilius",
    "Įrenginio bendrinimas",
    "Žaidimo nustatymai",
    "Garso takelių redaktorius",
    "Kreditai",
    "Minižaidimai",
    "Pasirinkti kalbą",
    "Anglų",
    "Kinų",
    "Lietuvių",
    "Japonų",
    "Garsas",
    "Daina",
    "Alyva",
    "kamuoliai",
    "PARDUOTUVĖ",
    "Aukščiausias rezultatas",
    "PRAKTIKA",
    "Tako ir kamuolio pasirinkimo žaidimas",
    "LAISVAS STILIUS",
    "Kova su atrakintais angelu, taku ir kamuoliu",
    "KITAS LYGIS",
    "BANDYTI DAR KARTĄ",
    "BONUSO ŽAIDIMAS",
    "ENERGIJA",
    "Ar norite tęsti boulingą?",
    "Sveiki atvykę į šį boulingo žaidimą.",
    "Spustelėkite PARUOŠTA, kad sufokusuotumėte žaidimo drobę.",
    "PARUOŠTA",
    "PASIRINKTI ANGELĄ",
    "PASIRINKTI BOTĄ",
    "PASIRINKTI TAKĄ",
    "KEISTI TAKĄ",
    "PASIRINKTI KAMUOLĮ",
    "INVENTORIUS",
    "PIRKTI DABAR",
    "Takai",
    "Pasirinkta: %s",
    "Pasirinkite taką",
    "Nėra takų",
    "Boto avataras",
    "Pasirinkite boto avatarą",
    "Nėra botų",
    "INVENTORIUS TUŠČIAS",
    "PARDUOTUVĖ TUŠČIA",
    "PARDUOTUVĖ: PAGERINK SAVO METIMĄ",
    "Dabartinis likutis",
    "UŽRAKINTA",
    "Atsistatys po %s",
    "Garso nustatymai",
    "Keičiama kokybė...",
    "Garso režimas",
    "SDL buferis",
    "Muzikos garsumas",
    "Aptiktas žemas našumas",
    "Pasirinkite parinktį:",
    "Naudoti sintezę",
    "Naudoti podėlį",
    "Išjungti",
    "Sintezė: Realaus laiko OPN lusto sintezė (be išankstinio įkėlimo, didesnė procesoriaus apkrova)",
    "Podėlis: Iš anksto sugeneruoti garso failai (reikia podėlio, mažesnė procesoriaus apkrova)",
    "Įrašoma į podėlį...",
    "Progresas: %d%% (%.1fs / %.1fs)",
    "Alyvos būsena",
    "Tako alyvos žemėlapis",
    "Tako informacija",
    "Maks. alyvos lygis: %.1fmm\nDabartinis: %.1fmm\nPernešimas: %.3fm/m\nSkilimas: %.4f/m",
    "Slidu",
    "Pertepti: NEMOKAMAI",
    "Pertepimo kaina: $%.0f (turite: $%.0f)",
    "Pertepimai: %d/%d",
    "PERTEPTI",
    "Nepakanka lėšų",
    "PERTEPIMAS UŽRAKINTAS",
    "ŠVIEŽIAI PERTEPTA",
    "Geriausi rezultatai",
    "Jūsų rezultatas tarp %d geriausių",
    "Puikus žaidimas!",
    "%d taškai",
    "Jūsų rezultatas didesnis nei %.0f%% visų pastarųjų žaidimų",
    "Lyderių lentelė (paskutinė valanda)",
    "Žaidėjas",
    "Rezultatas",
    "Laikas",
    "Dar nėra rezultatų – būk pirmas!",
    "Bandymai: %d",
    "Diapazonas: %d–%d",
    "Žaidimo nustatymai",
    "Snaigės: %d / %d",
    "Snaigių tankis",
    "Sniegas visiškai išjungtas, įskaitant atnaujinimus ir piešimo iškvietimus.",
    "ATSTATYTI PROGRESĄ",
    "Atstato kampaniją, atrakinimus ir pinigus. Vardas nesikeis.",
    "TIKRINTI ATNAUJINIMĄ",
    "ATNAUJINTI PWA",
    "IŠ NAUJO ĮKELTI SVETAINĘ",
    "Įdiegta versija: %s",
    "Paskelbta versija: %s",
    "Tikrinamas atnaujinimas...",
    "Naujausia versija jau įdiegta. Paskelbta versija: %s",
    "Yra atnaujinimas: %s -> %s",
    "Esate neprisijungę. Prisijunkite prie interneto ir bandykite dar kartą.",
    "Nepavyko pasiekti atnaujinimų serverio.",
    "Taikomas atnaujinimas...",
    "PWA atnaujinimų tikrinimas galimas tik žiniatinklio programoje.",
    "Žaidimo dizainas, programavimas, menas ir garsas: %s.",
    "Lapė Kalė",
    "Personažo pagrindas: Very Low Poly Human iš OpenGameArt.org.",
    "Personažų animacijos iš Mixamo.",
    "Sukurta naudojant SDL, OpenGL ES, Clay, GLM, Jolt Physics ir ymfm.",
    "Ypatinga padėka visiems, kurie žaidė, testavo ir metė dar vieną kartą.",
    "Pamoka %d. %s",
    "Taikymo pamoka",
    "Kamuolio masė",
    "Kamuolio sukimas",
    "Alyva ir slydimas",
    "Strike'o linija",
    "MASĖ (%.1fKG)",
    "Lengvo kamuolio testas",
    "Sunkaus kamuolio testas",
    "Išlaikyta",
    "%d / %d pataikymai",
    "MASĖ",
    "SUKIMASIS",
    "SLYDIMAS",
    "SUKIBIMAS",
    "PAPRASTAS",
    "RETAS",
    "EPINIS",
    "LEGENDINIS",
    "Sukimosi testas: %d/%d taikinių (Lygis %d/%d)",
    "Taikymo pamoka: %d/%d taškų",
    "Traukite kamuolį iki galo atgal",
    "Dabar paleiskite",
    "Eikite dešinėn",
    "Eikite kairėn",
    "Alyvos testas: pertepimai %d/%d",
    "Likusi alyva",
    "Pertepimo progresas",
    "Sudėvėkite alyvą prieš pertepdami.",
    "Strike testas: išmuškite STRIKE'ą",
    "1 LYGIS  PIRMASIS ŽINGSNIS",
    "Normalus biomas  Pasiek 100, kad pereitum",
    "2 LYGIS  MALACHAS ATVYKSTA",
    "Normalus biomas  Nugalėk Malachą",
    "3 LYGIS  DYKUMOS ĮSPĖJIMAS",
    "Dykumos biomas  Nugalėk Malachą",
    "4 LYGIS  STIKLINIS LEDAS",
    "Ledo biomas  Nugalėk Malachą",
    "5 LYGIS  NEONO STIKLO KLASĖ",
    "Neono biomas  Nugalėk Malach",
    "6 LYGIS  CHERUBEL NEONE",
    "Neono biomas  Nugalėk Cherubel",
    "7 LYGIS  JĖGOS SMŪGIO KLASĖ",
    "Normalus biomas  Nugalėk Cherubel",
    "8 LYGIS  SMĖLIO MEDIENA",
    "Dykumos biomas  Nugalėk Cherubel",
    "9 LYGIS  SERAPHEL KOPOSE",
    "Dykumos biomas  Nugalėk Seraphel",
    "10 LYGIS  LEDO AUDITORIJA",
    "Ledo biomas  Nugalėk Seraphel",
    "11 LYGIS  PLYTŲ IŠPAŽINTIS",
    "Neono biomas  Nugalėk Seraphel",
    "12 LYGIS  MIESTO RATAI",
    "Neono biomas  Nugalėk Thrones",
    "13 LYGIS  BETONO PARADAS",
    "Normalus biomas  Nugalėk Thrones",
    "Žarija",
    "Liepsna",
    "Ugnkablys",
    "Pragaras",
    "Feniksas",
    "Šerkšnas",
    "Vėsa",
    "Arktis",
    "Ledynas",
    "Pūga",
    "Žvaigždė",
    "Ūkas",
    "Kosmosas",
    "Tuštuma",
    "Juodoji",
    "NSO",
    "Ksenonas",
    "Miškas",
    "Vijoklis",
    "Perkūnas",
    "Titano šak.",
    "Pasaulmedis",
    "Šešėlis",
    "Bedugnė",
    "Anuliatorius",
    "Grandinė",
    "Neonas",
    "Kvantas",
    "Singuliarumas",
    "Banga",
    "Rifas",
    "Krakenas",
    "Leviatanas",
    "Runa",
    "Orakulas",
    "tu",
    "BONUSAS",
    "PAMOKA %d",
    "BAIGIMO LAIKAS %s",
    "Garso buferis padidintas iki 4096 dėl našumo",
    "suk, kad užlinktų",
    "NUMESK ČIA\nNAUDOTI",
    "DABAR\nNEGALIMA",
    "REIKIA BENT\n2 KAMUOLIŲ",
    "RUNA",
    "KAMUOLYS SPROGO",
    "KAMUOLYS IŠGARAVO",
    "KAMUOLYS ATSILAIKĖ",
    "KĖGLIAI UŽŠALO",
    "SARGAI IŠLEISTI",
    "FUTBOLAS ĮJUNGTAS",
    "ŽAIBAS ĮJUNGTAS",
    "KAUKOLĖ ĮJUNGTA",
    "LEDO KAMUOLYS",
    "SPROGIMAS ĮJUNGTAS",
    "NEPATAIKYTA",
    "SUSTOJO",
    "SKYLĖ",
    "STRIKE",
    "SPARE",
    "MEDIS",
    "PLYTA",
    "BETONAS",
    "STIKLAS",
    "Lobis",
    "BAKSTELK ATIDARYTI",
    "Gavai gebėjimą: %s",
    "Gavai %d monetų",
    "Tęsti",
    "Sprogimo runa",
    "Žaibo runa",
    "Šalčio runa",
    "Kaukolės runa",
    "Sargų runa",
    "Futbolo runa",
    "Runa",
    "Sprogstamas metimas. Galinga, bet rizikinga.",
    "Gynybos žaibas išgarina priešo kamuolį.",
    "Šaldo kėglius arba apledina tavo kamuolį.",
    "Kaukolė muša stipriau ir laužo blokus.",
    "Išleidžia tris sargus prieš varžovą.",
    "Atšoka kamuolį ir padaro jį greitesnį.",
    "Prideda gebėjimą į runų dėklą.",
    "Testinė prieiga. Atlygiai lieka, istorija ne.",
    "MONETŲ BĖGIMAS",
    "SKAIČIUOTOJAI",
    "MINIOS KONTROLĖ",
    "Žaisti %s?%s",
    " Prizo kamuolys: %s",
    "Atstatyti progresą",
    "Ar tikrai?",
    "Ištrina kampaniją, mokyklą, monetas, kamuolius ir atrakinimus.",
    "Perkrauna kampaniją, bet palieka kamuolius.",
    "Ne",
    "Taip",
    "Išeiti iš bonuso",
    "Nutraukti vykstantį bonuso lygį?",
    "Strike'ai",
    "Spare'ai",
    "Monetos",
    "Laimėta taškais",
    "Iš viso",
    "GAUTOS MONETOS",
    "PERGALĖ",
    "PRALAIMĖJAI",
    "TOLIAU",
    "DAR KARTĄ",
    "KARTOTI",
    "INVENTORIUS",
    "Papildymas po %s",
    "NETRUKUS",
    "BONUSO LYGIS",
    "ŽAISTI BONUSĄ",
    "BONUSAS BAIGTAS",
    "MONETOS: %d   UŽDIRBTA: $%d",
    "BONUSAS BAIGTAS  +$%d",
    "Išeita nebaigus.   UŽDIRBTA: $%d",
    "BONUSAS NUTRAUKTAS  +$%d",
    "BONUSO PERGALĖ",
    "BONUSO PRALAIMĖJIMAS",
    "BONUSAS PRALAIMĖTAS",
    "Pergalė: malachim pasiekė priešo bazę.",
    "Pralaimėjimas: priešai pasiekė tavo pradžią.",
    "Pergalė.",
    "Pralaimėjimas.",
    "   KAMUOLYS: %s",
    "Priešai",
    "Bosai",
    "Super bosas",
    "Vartai",
    "Kėgliai",
    "Likę",
    "ĮVEIKEI ANGELĄ",
    "ANGELAS LAIMI",
    "Tu: %d    Angelas: %d",
    "KAMPANIJA BAIGTA",
    "Visi lygiai įveikti. Takas tavo.",
    "Laikas  %s",
    "Bandymai  %d",
    "Bandymai pagal lygį",
    "L%d  %d",
    "Įvesti kodą",
    "kodas įjungtas",
    "tokio kodo nėra",
    "GREITESNIS SPAWNAS",
    "DAUGIAU JĖGOS",
    "%s EILĖ",
    "NITRO",
    "YRA BONUSAS",
    "PARDUOTUVĖ PAPILDYTA",
    "Yra naujų kamuolių. Apsilankyti parduotuvėje prieš kitą žaidimą?",
    "Į PARDUOTUVĘ",
    "VĖLIAU",
    "Įvesti tekstą",
    "Įvesti skaičių",
    "Trinti",
    "Įvesti",
    "Kirtis",
};

static constexpr const char *g_txl_chars_lt_lt = "MeniuSkčavmEt įoyląKbŽsPrLĮgdGųžAJpDRUOTVĖšzI,YBNĄę?.ėŠf:%Čj(c)1/ū\n34$0!–WĘ->wHxCh'2567Ū89ŲF+";

static constexpr const char *g_txl_jp_jp[TXL_KEY_COUNT] = {
    "メニュー",
    "チート",
    "学校へ行く",
    "言語",
    "プレイ",
    "練習",
    "フリースタイル",
    "デバイス共有",
    "ゲーム設定",
    "トラッカー",
    "クレジット",
    "ミニゲーム",
    "言語選択",
    "英語",
    "中国語",
    "リトアニア語",
    "日本語",
    "サウンド",
    "曲",
    "オイル",
    "ボール",
    "ショップ",
    "ハイスコア",
    "練習",
    "レーンとボールを選ぶ練習",
    "フリースタイル",
    "解除済みのエンジェル、レーン、ボールで対戦",
    "次のレベル",
    "もう一度",
    "ボーナスゲーム",
    "エネルギー",
    "ボウリングを続けますか？",
    "このボウリングゲームへようこそ。",
    "「レディー」を押してゲーム画面に戻ります。",
    "レディー",
    "エンジェルを選択",
    "対戦相手を選択",
    "レーンを選択",
    "レーンを切り替え",
    "ボールを選択",
    "所持ボール",
    "今すぐ購入",
    "レーン",
    "選択中: %s",
    "レーンを選んでください",
    "レーンがありません",
    "対戦相手のアバター",
    "対戦相手のアバターを選択",
    "対戦相手がいません",
    "所持ボールがありません",
    "ショップは空です",
    "ショップ: ゲームを強化",
    "現在の残高",
    "ロック中",
    "%s にリセット",
    "サウンド設定",
    "品質を変更中...",
    "オーディオモード",
    "SDL バッファ",
    "音楽音量",
    "パフォーマンス低下を検出",
    "オプションを選択してください：",
    "シンセを使用",
    "キャッシュを使用",
    "無効にする",
    "シンセ：リアルタイムOPNチップ合成（プリロードなし、CPU負荷高め）",
    "キャッシュ：事前生成オーディオ（キャッシュ必要、CPU負荷低め）",
    "オーディオをキャッシュ中...",
    "進捗: %d%% (%.1f秒 / %.1f秒)",
    "オイル状態",
    "レーンオイルマップ",
    "トラック情報",
    "最大オイル量: %.1fmm\n現在: %.1fmm\nキャリーダウン: %.3fm/m\n減衰: %.4f/m",
    "滑りやすさ",
    "再オイル: 無料",
    "再オイル費用: $%.0f (所持金: $%.0f)",
    "再オイル: %d/%d",
    "再オイル",
    "資金不足",
    "再オイルはロックされています",
    "オイル済み",
    "トップスコア",
    "あなたのスコアはトップ%dに入っています",
    "良いゲーム！",
    "%dポイント",
    "あなたのスコアは最近の全プレイの%.0f%%を上回っています",
    "リーダーボード（直近1時間）",
    "プレイヤー",
    "スコア",
    "時間",
    "まだスコアがありません - 最初に記録しよう！",
    "試行回数: %d",
    "範囲: %d～%d",
    "ゲーム設定",
    "雪片: %d / %d",
    "雪の密度",
    "雪は完全に無効（更新・描画を含む）",
    "進行状況をリセット",
    "キャンペーン、解除、所持金をリセット。名前はそのまま。",
    "更新を確認",
    "PWA を更新",
    "ウェブサイトを再読み込み",
    "インストール済みビルド: %s",
    "公開ビルド: %s",
    "更新を確認中...",
    "最新です。公開ビルド: %s",
    "更新があります: %s -> %s",
    "オフラインです。ネット接続後に再確認してください。",
    "更新サーバーに接続できませんでした。",
    "更新を適用しています...",
    "PWA 更新確認はウェブ版でのみ利用できます。",
    "ゲームデザイン、プログラム、アート、サウンド: %s。",
    "Lape Kale",
    "キャラクター基礎モデル: OpenGameArt.org の Very Low Poly Human。",
    "キャラクターアニメーション: Mixamo。",
    "SDL、OpenGL ES、Clay、GLM、Jolt Physics、ymfm を使用。",
    "プレイ、テスト、そしてもう1フレーム投げ続けてくれた皆さんに感謝します。",
    "レッスン %d. %s",
    "エイムレッスン",
    "ボールの質量",
    "ボールに回転をかける",
    "オイルとスキッド",
    "ストライクライン",
    "質量 (%.1fkg)",
    "軽いボールのテスト",
    "重いボールのテスト",
    "合格",
    "%d / %d ヒット",
    "質量",
    "回転",
    "スキッド",
    "バイト",
    "コモン",
    "レア",
    "エピック",
    "レジェンダリー",
    "スピンテスト: %d/%d ターゲット (レベル %d/%d)",
    "エイムレッスン: %d/%d ポイント",
    "ボールを一番後ろまで引いてください",
    "今すぐ放してください",
    "右に移動",
    "左に移動",
    "オイルテスト: 再オイル %d/%d",
    "残りのオイル",
    "再オイルの進行状況",
    "再オイルする前に使い切ってください。",
    "ストライクテスト: ストライクを取れ",
    "レベル1 最初の目標",
    "ノーマルバイオーム 100点でクリア",
    "レベル2 マラック登場",
    "ノーマルバイオーム マラックに勝利",
    "レベル3 砂漠の警告",
    "砂漠バイオーム マラックに勝利",
    "レベル4 ガラスの氷",
    "氷バイオーム マラックに勝利",
    "レベル5 ネオン硝子",
    "ネオン  Malachに勝利",
    "レベル6 Cherubel",
    "ネオン  Cherubelに勝利",
    "レベル7 パワー教室",
    "ノーマル  Cherubelに勝利",
    "レベル8 砂の材木",
    "砂漠  Cherubelに勝利",
    "レベル9 砂丘のSeraphel",
    "砂漠  Seraphelに勝利",
    "レベル10 氷の観客",
    "氷  Seraphelに勝利",
    "レベル11 レンガ告白",
    "ネオン  Seraphelに勝利",
    "レベル12 街の車輪",
    "ネオン  Thronesに勝利",
    "レベル13 コンクリート",
    "ノーマル  Thronesに勝利",
    "残り火ストライク",
    "炎ロール",
    "ブレイズフック",
    "インフェルノ",
    "フェニックス",
    "フロストグライド",
    "チルストライク",
    "北極ドリフト",
    "氷河バイト",
    "吹雪キング",
    "星くず",
    "星雲ロール",
    "宇宙フック",
    "虚空ストライク",
    "ブラックホール",
    "UFOスピン",
    "キセノンバイト",
    "森ロール",
    "ツタストライク",
    "雷オーク",
    "巨人の根",
    "世界樹",
    "影ドリフト",
    "深淵フック",
    "ヌリファイア",
    "回路ロール",
    "ネオンストライク",
    "量子フック",
    "特異点",
    "潮ロール",
    "リーフストライク",
    "クラーケンバイト",
    "リヴァイアサン",
    "ルーンボール",
    "オラクルストライク",
    "あなた",
    "ボーナス",
    "レッスン%d",
    "クリア時間 %s",
    "負荷対策で音声バッファを4096に変更",
    "回して曲げる",
    "ここへ\nドロップ",
    "今は\n使えません",
    "ボールが\n2個必要",
    "ルーン",
    "ボール爆散",
    "ボール消滅",
    "ボール生存",
    "ピン凍結",
    "警備ピン配置",
    "フットボール起動",
    "フラッシュ起動",
    "スカル起動",
    "アイスボール起動",
    "ブーム起動",
    "ミス",
    "停止",
    "スプリット",
    "ストライク",
    "スペア",
    "木",
    "レンガ",
    "コンクリ",
    "ガラス",
    "宝箱",
    "タップで開く",
    "能力を入手: %s",
    "%dコイン入手",
    "続ける",
    "ブームルーン",
    "稲妻ルーン",
    "氷結ルーン",
    "スカルルーン",
    "警備ピンルーン",
    "フットボールルーン",
    "ルーン",
    "爆発ショット。強力だがボールを失う危険あり。",
    "防衛時、稲妻で敵ボールを消す。",
    "防衛でピン凍結、攻撃で氷ボール化。",
    "スカル球。強打とブロック破壊が得意。",
    "警備ピン3本で敵を妨害。",
    "球を跳ねさせ、軽く速くする。",
    "ルーントレイに能力を追加。",
    "テスト入口。報酬あり、物語進行なし。",
    "コインラッシュ",
    "カウントマスター",
    "群衆コントロール",
    "%sを遊びますか？%s",
    " 報酬ボール: %s",
    "進行リセット",
    "よろしいですか？",
    "進行、学校、コイン、ボール、解除を消去します。",
    "キャンペーンを再開。ボールは残ります。",
    "いいえ",
    "はい",
    "ボーナス終了",
    "進行中のボーナスを終了しますか？",
    "ストライク",
    "スペア",
    "コイン",
    "点差勝利",
    "合計",
    "獲得コイン",
    "勝利",
    "敗北",
    "次へ",
    "再挑戦",
    "リピート",
    "所持品",
    "補充まで %s",
    "まもなく補充",
    "ボーナス",
    "ボーナス開始",
    "ボーナス完了",
    "コイン: %d   獲得: $%d",
    "ボーナス完了  +$%d",
    "途中終了。獲得: $%d",
    "ボーナス終了  +$%d",
    "ボーナス勝利",
    "ボーナス敗北",
    "ボーナス失敗",
    "勝利: malachimが敵基地に到達。",
    "敗北: 敵が出現地点に到達。",
    "勝利。",
    "敗北。",
    "   ボール: %s",
    "敵",
    "ボス",
    "強ボス",
    "ゲート",
    "ピン",
    "残り",
    "天使に勝利",
    "天使の勝利",
    "あなた: %d    天使: %d",
    "キャンペーン完了",
    "全レベルクリア。レーンはあなたのもの。",
    "合計時間  %s",
    "挑戦  %d",
    "レベル別挑戦",
    "L%d  %d",
    "チート入力",
    "チート有効",
    "該当チートなし",
    "出現が速い",
    "強くなる",
    "%sの番",
    "NOS",
    "ボーナスあり",
    "ショップ更新",
    "新しいボールが入荷。次のゲーム前にショップへ行きますか？",
    "ショップへ",
    "あとで",
    "テキスト入力",
    "数値入力",
    "削除",
    "決定",
    "アクセント",
};

static constexpr const char *g_txl_chars_jp_jp = "メニューチト学校へ行く言語プレイ練習フリスタルデバ共有ゲム設定ラッカクジミ選択英中国ア日本サウンド曲オボショハコとをぶ解除済みのエェ、で対戦次ベもう一度ナネギグ続けますか？こよそ。「ィ」押して画面に戻り相手切替え所持今ぐ購入: %sんださいがあせは空強化現在残高ロセ品質変更.モSDLァ音楽量パォマ低下検出：使用キャ無効るOPN合成（なCU負荷め）事前生必要進捗d(1f秒/)状態情報最大m\nダ3減衰4滑や再料費$0金資不足れたっ良！ポ近全上回直時間ヤ-初記録試数範囲～雪片密完新・描含む況ペ名確認WAブ読込ビ公開>接後き適版利ザapeKl基礎nGrtogVywHuMixEJhcテ投げ皆感謝転k軽重格ヒピ番ろ引放右移動左取目標ノ点2登場勝砂漠警告ガ氷5硝子6b7ワ教室8材木9丘観客白街車輪T火炎ズ北極河吹星ず雲宇宙虚ホF森ツ雷巨人根世界樹影深淵ヌ路特異潮ケヴ策声個爆散消滅存凍結備配置起停止宝箱能力稲妻発失危険防衛敵攻撃球打破壊得意妨害跳ね速追加口酬物群衆遊び去終了差計獲敗挑補充始+途地到達天別該当値削決ė";

static constexpr const char *g_txl_zh_cn[TXL_KEY_COUNT] = {
    "菜单",
    "作弊",
    "去学校",
    "语言",
    "开始",
    "练习",
    "自由赛",
    "设备共享",
    "游戏设置",
    "音轨编辑",
    "制作名单",
    "小游戏",
    "选择语言",
    "English",
    "中文",
    "立陶宛语",
    "日语",
    "声音",
    "歌曲",
    "油道",
    "球库",
    "商店",
    "最高分",
    "练习",
    "选择球道和球的练习局",
    "自由赛",
    "使用已解锁的天使、球道和球对战",
    "下一关",
    "再试一次",
    "奖励关",
    "能量",
    "你想继续打保龄球吗？",
    "欢迎来到这款保龄球游戏。",
    "点击“准备”以聚焦游戏画面。",
    "准备",
    "选择天使",
    "选择对手",
    "选择球道",
    "切换球道",
    "选择球",
    "库存",
    "立即购买",
    "球道",
    "已选择：%s",
    "选择一个球道",
    "没有球道",
    "对手形象",
    "选择一个对手形象",
    "没有对手",
    "库存为空",
    "商店已空",
    "商店：强化这一局",
    "当前余额",
    "未解锁",
    "%s后重置",
    "声音设置",
    "正在切换音频模式...",
    "音频模式",
    "SDL 缓冲",
    "音乐音量",
    "检测到性能偏低",
    "请选择一个选项：",
    "使用合成",
    "使用缓存",
    "关闭音频",
    "合成：实时 OPN 芯片合成（无需预载，CPU 占用更高）",
    "缓存：预生成音频数据（需要缓存，CPU 占用更低）",
    "正在缓存音频...",
    "进度：%d%%（%.1f秒 / %.1f秒）",
    "油道状态",
    "油图",
    "球道信息",
    "最大油量：%.1f毫米\n当前：%.1f毫米\n带油：%.3f米/米\n衰减：%.4f/米",
    "滑度",
    "重新上油：免费",
    "上油：$%.0f（有$%.0f）",
    "重新上油：%d/%d",
    "重新上油",
    "余额不足",
    "暂时不能上油",
    "刚上过油",
    "最高分",
    "你的分数进入前%d名",
    "不错的一局！",
    "%d分",
    "你的分数超过了近期%.0f%%的对局",
    "排行榜（最近一小时）",
    "玩家",
    "分数",
    "时间",
    "还没有分数，来当第一个吧！",
    "尝试次数：%d",
    "范围：%d-%d",
    "游戏设置",
    "雪花：%d / %d",
    "雪花密度",
    "雪已完全关闭，包括更新和绘制。",
    "重置进度",
    "重置战役、解锁和现金。用户名不变。",
    "检查更新",
    "更新 PWA",
    "重新加载网站",
    "已安装构建：%s",
    "已发布构建：%s",
    "正在检查更新……",
    "已是最新。已发布构建：%s",
    "有可用更新：%s -> %s",
    "当前离线。请连接网络后检查更新。",
    "无法连接更新服务器。",
    "正在应用更新……",
    "只有网页应用支持 PWA 更新检查。",
    "游戏设计、程序、美术和声音：%s。",
    "Lape Kale",
    "角色基础模型：OpenGameArt.org 的 Very Low Poly Human。",
    "角色动画来自 Mixamo。",
    "使用 SDL、OpenGL ES、Clay、GLM、Jolt Physics 和 ymfm 构建。",
    "特别感谢每一位游玩、测试，并继续多投一局的人。",
    "第%d课：%s",
    "瞄准课",
    "球的质量",
    "给球上旋",
    "油与滑行",
    "全中线路",
    "质量（%.1f公斤）",
    "轻球测试",
    "重球测试",
    "已通过",
    "%d / %d 次击中",
    "质量",
    "旋转",
    "滑行",
    "咬道",
    "普通",
    "稀有",
    "史诗",
    "传说",
    "旋转测试：%d/%d 目标（第%d/%d关）",
    "瞄准课：%d/%d分",
    "把球尽量向后拉",
    "现在放手",
    "向右移动",
    "向左移动",
    "油道测试：重新上油 %d/%d",
    "剩余油量",
    "上油进度",
    "先把油耗掉，再重新上油。",
    "全中测试：打出STRIKE",
    "第1关  第一个里程碑",
    "普通球道  达到100分即可通过",
    "第2关  玛拉克到来",
    "普通球道  击败玛拉克",
    "第3关  沙漠警告",
    "沙漠球道  击败玛拉克",
    "第4关  玻璃冰面",
    "冰面球道  击败玛拉克",
    "第5关  霓虹玻璃课",
    "霓虹球道  击败Malach",
    "第6关  Cherubel在霓虹",
    "霓虹球道  击败Cherubel",
    "第7关  强力球课",
    "普通球道  击败Cherubel",
    "第8关  沙地木障",
    "沙漠球道  击败Cherubel",
    "第9关  沙丘中的Seraphel",
    "沙漠球道  击败Seraphel",
    "第10关  冰上会面",
    "冰面球道  击败Seraphel",
    "第11关  砖块告白",
    "霓虹球道  击败Seraphel",
    "第12关  城市之轮",
    "霓虹球道  击败Thrones",
    "第13关  混凝土游行",
    "普通球道  击败Thrones",
    "余烬强击",
    "烈焰滚",
    "火焰钩",
    "炼狱怒",
    "凤凰升",
    "霜滑",
    "寒意强击",
    "极地漂移",
    "冰川咬道",
    "暴雪王",
    "星尘",
    "星云滚",
    "宇宙钩",
    "虚空强击",
    "黑洞",
    "UFO旋",
    "氙光咬道",
    "森林滚",
    "藤蔓强击",
    "雷橡",
    "泰坦根",
    "世界树",
    "暗影漂移",
    "深渊钩",
    "归零者",
    "电路滚",
    "霓虹强击",
    "量子钩",
    "奇点",
    "潮汐滚",
    "礁石强击",
    "海妖咬道",
    "利维坦",
    "符文球",
    "神谕强击",
    "你",
    "奖励局",
    "课程 %d",
    "通关时间 %s",
    "性能偏低，音频缓冲已调至4096",
    "旋转可弯球",
    "拖到这里\n使用",
    "现在\n不能用",
    "至少需要\n2个球",
    "符文",
    "球已爆炸",
    "球已蒸发",
    "球扛过闪光",
    "球瓶冻结",
    "巡逻瓶已部署",
    "足球已激活",
    "闪光已激活",
    "骷髅已激活",
    "冰球已激活",
    "爆炸已激活",
    "未命中",
    "停住",
    "分瓶",
    "全中",
    "补中",
    "木块",
    "砖块",
    "混凝土",
    "玻璃",
    "宝箱",
    "点击打开宝箱",
    "获得能力：%s",
    "获得%d枚金币",
    "继续",
    "爆炸符文",
    "闪电符文",
    "冰冻符文",
    "骷髅符文",
    "巡逻瓶符文",
    "足球符文",
    "符文",
    "爆炸投球，冲击强但有失球风险。",
    "防守闪电会蒸发敌方球。",
    "防守冻瓶，进攻让球变冰。",
    "骷髅球：击瓶更狠，破障更强。",
    "部署三个巡逻瓶阻挡对手。",
    "立刻弹起球，并让它更轻更快。",
    "向符文栏添加一个能力。",
    "测试入口。奖励有效，故事不推进。",
    "金币冲刺",
    "数数大师",
    "人群控制",
    "要玩%s吗？%s",
    " 奖励球：%s",
    "重置进度",
    "确定吗？",
    "会清除战役、学校、金币、球和解锁。",
    "重开战役，但保留球库。",
    "否",
    "是",
    "退出奖励关",
    "要退出进行中的奖励关吗？",
    "全中",
    "补中",
    "金币",
    "分数胜利",
    "合计",
    "获得金币",
    "胜利",
    "失败",
    "下一步",
    "重试",
    "再来",
    "库存",
    "%s后补货",
    "即将补货",
    "奖励关",
    "开始奖励关",
    "奖励局完成",
    "拾取金币：%d   获得：$%d",
    "奖励完成  +$%d",
    "中途退出。已获得：$%d",
    "奖励局已退出  +$%d",
    "奖励局胜利",
    "奖励局失败",
    "奖励局失败",
    "胜利：malachim到达敌方基地。",
    "失败：敌人到达你的出生点。",
    "胜利。",
    "失败。",
    "   球：%s",
    "敌人",
    "首领",
    "超级首领",
    "门",
    "球瓶",
    "剩余",
    "你击败了天使",
    "天使获胜",
    "你：%d    天使：%d",
    "战役完成",
    "所有关卡已清。球道归你了。",
    "总时间  %s",
    "尝试  %d",
    "各关尝试",
    "L%d  %d",
    "输入作弊码",
    "作弊已启用",
    "没有这个作弊码",
    "生成更快",
    "力量更强",
    "%s回合",
    "氮气",
    "奖励可用",
    "商店已更新",
    "新球已上架。下局前要去商店看看吗？",
    "去商店",
    "稍后",
    "输入文字",
    "输入数字",
    "删除",
    "确认",
    "重音",
};

static constexpr const char *g_txl_chars_zh_cn = "菜单作弊去学校语言开始练习自由赛设备共享游戏置音轨编辑制名小选择English中文立陶宛日声歌曲油道球库商店最高分和的局使用已解锁天、对战下一关再试次奖励能量你想继续打保龄吗？欢迎来到这款。点击“准”以聚焦画面手切换存即购买：%个没有形象为空强化当前余额未后重正在频模式.SDL 缓冲乐检测性偏低请项合成闭实时OPN芯片（无需预载，CU占更）生数据要进度d1f秒/状态图信息大毫米\n带3衰减4滑新上免费$0不足暂刚过入错！超了近期排行榜玩家间还第吧尝范围-雪花密完全包括绘役现金户变查WA加网站安装构建发布…是可>离线连接络法服务器应只页支持计程序美术apeK角色基础型GmrtoVywHu动MxJc特别感谢每位并多投人课瞄质给旋与路公斤轻通转咬普稀史诗传说目标把尽向拉放右移左剩先耗掉出TRI里碑达2玛克败沙漠警告玻璃冰5霓虹6b7力8地木障9丘会砖块白城市之轮混凝土烬烈焰滚火钩炼狱怒凤凰升霜寒意极漂川暴王星尘云宇宙虚黑洞F氙光森林藤蔓雷橡泰坦根世界树暗影深渊归零者电子奇潮汐礁石海妖利维符神谕调至弯拖少爆炸蒸扛闪瓶冻结巡逻部署激活骷髅命停住补宝箱获得枚币但失风险防守敌方攻让狠破三阻挡刻弹起它快栏添口效故事推刺师群控确定清除留否退胜步货将拾取+途首领级门所卡总各输码启回氮气架看稍字删认ė";

