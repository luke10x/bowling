#pragma once

// Generated by scripts/generate_txl.py

enum TxlKey
{
    TXL_MENU,
    TXL_RENAME,
    TXL_GO_TO_SCHOOL,
    TXL_LANGUAGE,
    TXL_CAMPAIGN,
    TXL_PRACTICE,
    TXL_FREESTYLE,
    TXL_DEVICE_SHARE,
    TXL_GAME_SETTINGS,
    TXL_TRACKER,
    TXL_SELECT_LANGUAGE,
    TXL_LANGUAGE_ENGLISH,
    TXL_LANGUAGE_CHINESE,
    TXL_LANGUAGE_LITHUANIAN,
    TXL_LANGUAGE_JAPANESE,
    TXL_SOUND,
    TXL_SONG,
    TXL_OIL,
    TXL_SHOP,
    TXL_HI_SCORE,
    TXL_PRACTICE_TITLE,
    TXL_PRACTICE_SUBTITLE,
    TXL_FREESTYLE_TITLE,
    TXL_FREESTYLE_SUBTITLE,
    TXL_NEXT_LEVEL,
    TXL_TRY_AGAIN,
    TXL_BALL_CHARGE_FMT,
    TXL_GREETING_RESUME,
    TXL_GREETING_WELCOME,
    TXL_GREETING_SUBLINE,
    TXL_READY,
    TXL_SELECT_ANGEL,
    TXL_SELECT_BOT,
    TXL_SELECT_HOUSE,
    TXL_SWITCH_HOUSE,
    TXL_SELECT_BALL,
    TXL_BUY_NOW,
    TXL_HOUSES,
    TXL_SELECTED_FMT,
    TXL_SELECT_A_HOUSE,
    TXL_NO_HOUSES,
    TXL_BOT_AVATAR,
    TXL_SELECT_A_BOT_AVATAR,
    TXL_NO_BOTS,
    TXL_SHOP_IMPROVE_YOUR_RUN,
    TXL_CURRENT_BALANCE,
    TXL_LOCKED,
    TXL_RESETS_IN_FMT,
    TXL_SOUND_SETTINGS,
    TXL_CHANGING_QUALITY,
    TXL_AUDIO_MODE,
    TXL_SDL_BUFFER,
    TXL_MUSIC_VOLUME,
    TXL_LOW_PERFORMANCE_DETECTED,
    TXL_PLEASE_CHOOSE_OPTION,
    TXL_USE_SYNTH,
    TXL_USE_CACHED,
    TXL_DISABLE,
    TXL_SYNTH_EXPLAIN,
    TXL_CACHED_EXPLAIN,
    TXL_CACHING_AUDIO,
    TXL_PROGRESS_FMT,
    TXL_OIL_STATUS,
    TXL_LANE_OIL_MAP,
    TXL_TRACK_INFO,
    TXL_MAX_OIL_LEVEL_FMT,
    TXL_SLIPPERY,
    TXL_REOIL_FREE,
    TXL_REOIL_COST_FMT,
    TXL_REOILS_FMT,
    TXL_REOIL,
    TXL_CANT_AFFORD,
    TXL_REOIL_LOCKED,
    TXL_TOP_SCORES,
    TXL_YOUR_SCORE_TOP_FMT,
    TXL_GOOD_RUN,
    TXL_POINTS_FMT,
    TXL_SCORE_HIGHER_THAN_FMT,
    TXL_LEADERBOARD_LAST_HOUR,
    TXL_PLAYER,
    TXL_SCORE,
    TXL_AGE,
    TXL_NO_SCORES_YET,
    TXL_ATTEMPTS_FMT,
    TXL_RANGE_FMT,
    TXL_GAME_SETTINGS_TITLE,
    TXL_SNOWFLAKES_FMT,
    TXL_SNOWFLAKE_DENSITY,
    TXL_SNOW_DISABLED,
    TXL_RESET_PROGRESS,
    TXL_RESET_PROGRESS_HELP,
    TXL_SCHOOL_TITLE_FMT,
    TXL_AIM_LESSON,
    TXL_BALL_MASS,
    TXL_SPIN_BALL,
    TXL_OIL_AND_SKID,
    TXL_STRIKE_LINE,
    TXL_MASS_FMT,
    TXL_LIGHT_BALL_TEST,
    TXL_HEAVY_BALL_TEST,
    TXL_PASSED,
    TXL_HITS_FMT,
    TXL_MASS,
    TXL_SPIN,
    TXL_SKID,
    TXL_BITE,
    TXL_RARITY_COMMON,
    TXL_RARITY_RARE,
    TXL_RARITY_EPIC,
    TXL_RARITY_LEGENDARY,
    TXL_SPIN_TEST_FMT,
    TXL_AIM_LESSON_POINTS_FMT,
    TXL_PULL_BALL_BACK,
    TXL_LET_IT_GO_NOW,
    TXL_MOVE_RIGHT,
    TXL_MOVE_LEFT,
    TXL_OIL_TEST_REOILS_FMT,
    TXL_REMAINING_OIL,
    TXL_REOIL_PROGRESS,
    TXL_WEAR_IT_DOWN,
    TXL_STRIKE_TEST_SCORE_STRIKE,
    TXL_LEVEL1_TITLE,
    TXL_LEVEL1_SUBTITLE,
    TXL_LEVEL2_TITLE,
    TXL_LEVEL2_SUBTITLE,
    TXL_LEVEL3_TITLE,
    TXL_LEVEL3_SUBTITLE,
    TXL_LEVEL4_TITLE,
    TXL_LEVEL4_SUBTITLE,
    TXL_LEVEL5_TITLE,
    TXL_LEVEL5_SUBTITLE,
    TXL_LEVEL6_TITLE,
    TXL_LEVEL6_SUBTITLE,
    TXL_LEVEL7_TITLE,
    TXL_LEVEL7_SUBTITLE,
    TXL_LEVEL8_TITLE,
    TXL_LEVEL8_SUBTITLE,
    TXL_LEVEL9_TITLE,
    TXL_LEVEL9_SUBTITLE,
    TXL_LEVEL10_TITLE,
    TXL_LEVEL10_SUBTITLE,
    TXL_LEVEL11_TITLE,
    TXL_LEVEL11_SUBTITLE,
    TXL_KEY_COUNT
};

static constexpr const char *g_txl_en_us[TXL_KEY_COUNT] = {
    "Menu",
    "Rename",
    "Go to school",
    "Language",
    "Campaign",
    "Practice",
    "Freestyle",
    "Device Share",
    "Game Settings",
    "Tracker",
    "Select Language",
    "English",
    "Chinese",
    "Lithuanian",
    "Japanese",
    "Sound",
    "Song",
    "Oil",
    "Shop",
    "Hi-Score",
    "PRACTICE",
    "House and ball selection run",
    "FREESTYLE",
    "Versus with your unlocked angel, house, and ball",
    "NEXT LEVEL",
    "TRY AGAIN",
    "BALL CHARGE %d%%",
    "Do you want to continue to play bowling?",
    "Welcome to this bowling game.",
    "Click READY to focus the game canvas.",
    "READY",
    "SELECT ANGEL",
    "SELECT BOT",
    "SELECT HOUSE",
    "SWITCH HOUSE",
    "SELECT BALL",
    "BUY NOW",
    "Houses",
    "Selected: %s",
    "Select a house",
    "No houses",
    "Bot Avatar",
    "Select a bot avatar",
    "No bots",
    "SHOP: IMPROVE YOUR RUN",
    "Current balance",
    "LOCKED",
    "Resets in %s",
    "Sound Settings",
    "Changing quality...",
    "Audio Mode",
    "SDL Buffer",
    "Music Volume",
    "Low Performance Detected",
    "Please choose an option:",
    "Use Synth",
    "Use Cached",
    "Disable",
    "Synth: Real-time OPN chip synthesis (no preload, more CPU)",
    "Cached: Pre-generated audio blobs (needs caching, lighter on CPU)",
    "Caching Audio...",
    "Progress: %d%% (%.1fs / %.1fs)",
    "Oil Status",
    "Lane Oil Map",
    "Track Info",
    "Max oil level: %.1fmm\nCurrent: %.1fmm\nCarrydown: %.3fm/m\nDecay: %.4f/m",
    "Slippery",
    "Re-oil: FREE",
    "Re-oil cost: $%.0f  (you have: $%.0f)",
    "Re-oils: %d/%d",
    "RE-OIL",
    "CAN'T AFFORD",
    "RE-OIL LOCKED",
    "Top Scores",
    "Your score is in top %d",
    "Good Run!",
    "%d points",
    "Your score is higher than %.0f%% of all recent runs",
    "Leaderboard (Last Hour)",
    "Player",
    "Score",
    "Age",
    "No scores yet - be the first!",
    "Attempts: %d",
    "Range: %d-%d",
    "Game Settings",
    "Snowflakes: %d / %d",
    "Snowflake density",
    "Snow is fully disabled, including updates and draw calls.",
    "RESET PROGRESS",
    "Resets campaign level, unlocks, and cash. Username stays unchanged.",
    "School :: Lesson %d. %s",
    "Aim lesson",
    "Ball Mass",
    "Spin ball",
    "Oil and skid",
    "Strike line",
    "MASS (%.1fKG)",
    "Light ball test",
    "Heavy ball test",
    "Passed",
    "%d / %d hits",
    "MASS",
    "SPIN",
    "SKID",
    "BITE",
    "COMMON",
    "RARE",
    "EPIC",
    "LEGENDARY",
    "Spin test: %d/%d coins (Level %d/%d)",
    "Aim lesson: %d/%d points",
    "Pull the ball all the way back",
    "Let it go now",
    "Move right",
    "Move left",
    "Oil test: re-oils %d/%d",
    "Remaining oil",
    "Re-oil progress",
    "Wear it down before re-oiling.",
    "Strike test: score a STRIKE",
    "LEVEL 1  FIRST MILESTONE",
    "Normal biome  Reach 100 to pass",
    "LEVEL 2  MALACH ARRIVES",
    "Normal biome  Beat Malach",
    "LEVEL 3  DESERT WARNING",
    "Desert biome  Beat Malach",
    "LEVEL 4  GLASS ICE",
    "Ice biome  Beat Malach",
    "LEVEL 5  DOG'S CHALLENGE",
    "Normal biome  Beat Dog",
    "LEVEL 6  POWER SHOT ALLEY",
    "Neon biome  Beat Dog",
    "LEVEL 7  SAND TALK",
    "Desert biome  Beat Dog",
    "LEVEL 8  BEAK IN THE DUNES",
    "Desert biome  Beat Beak",
    "LEVEL 9  ICE AUDIENCE",
    "Ice biome  Beat Beak",
    "LEVEL 10  NEON CONFESSION",
    "Neon biome  Beat Beak",
    "LEVEL 11  WHEELS OF THE CITY",
    "Neon biome  Beat Cow",
};

static constexpr const char *g_txl_chars_en_us = "MenuRamGo tschlLgCpiPrFyDvSTkEJdOH-AIbYVw,NXB%?W.fU:Kq()1/x\n34$0'!256789";

static constexpr const char *g_txl_lt_lt[TXL_KEY_COUNT] = {
    "Meniu",
    "Pervadinti",
    "Eiti į mokyklą",
    "Kalba",
    "Kampanija",
    "Praktika",
    "Laisvas stilius",
    "Įrenginio bendrinimas",
    "Žaidimo nustatymai",
    "Garso takelių redaktorius",
    "Pasirinkti kalbą",
    "Anglų",
    "Kinų",
    "Lietuvių",
    "Japonų",
    "Garsas",
    "Daina",
    "Alyva",
    "Parduotuvė",
    "Aukščiausias rezultatas",
    "PRAKTIKA",
    "Tako ir kamuolio pasirinkimo žaidimas",
    "LAISVAS STILIUS",
    "Kova su atrakintais angelu, taku ir kamuoliu",
    "KITAS LYGIS",
    "BANDYTI DAR KARTĄ",
    "KAMUOLIO ĮKROVA %d%%",
    "Ar norite tęsti boulingą?",
    "Sveiki atvykę į šį boulingo žaidimą.",
    "Spustelėkite PARUOŠTA, kad sufokusuotumėte žaidimo drobę.",
    "PARUOŠTA",
    "PASIRINKTI ANGELĄ",
    "PASIRINKTI BOTĄ",
    "PASIRINKTI TAKĄ",
    "KEISTI TAKĄ",
    "PASIRINKTI KAMUOLĮ",
    "PIRKTI DABAR",
    "Takai",
    "Pasirinkta: %s",
    "Pasirinkite taką",
    "Nėra takų",
    "Boto avataras",
    "Pasirinkite boto avatarą",
    "Nėra botų",
    "PARDUOTUVĖ: PAGERINK SAVO METIMĄ",
    "Dabartinis likutis",
    "UŽRAKINTA",
    "Atsistatys po %s",
    "Garso nustatymai",
    "Keičiama kokybė...",
    "Garso režimas",
    "SDL buferis",
    "Muzikos garsumas",
    "Aptiktas žemas našumas",
    "Pasirinkite parinktį:",
    "Naudoti sintezę",
    "Naudoti podėlį",
    "Išjungti",
    "Sintezė: Realaus laiko OPN lusto sintezė (be išankstinio įkėlimo, didesnė procesoriaus apkrova)",
    "Podėlis: Iš anksto sugeneruoti garso failai (reikia podėlio, mažesnė procesoriaus apkrova)",
    "Įrašoma į podėlį...",
    "Progresas: %d%% (%.1fs / %.1fs)",
    "Alyvos būsena",
    "Tako alyvos žemėlapis",
    "Tako informacija",
    "Maks. alyvos lygis: %.1fmm\nDabartinis: %.1fmm\nPernešimas: %.3fm/m\nSkilimas: %.4f/m",
    "Slidu",
    "Pertepti: NEMOKAMAI",
    "Pertepimo kaina: $%.0f (turite: $%.0f)",
    "Pertepimai: %d/%d",
    "PERTEPTI",
    "Nepakanka lėšų",
    "PERTEPIMAS UŽRAKINTAS",
    "Geriausi rezultatai",
    "Jūsų rezultatas tarp %d geriausių",
    "Puikus žaidimas!",
    "%d taškai",
    "Jūsų rezultatas didesnis nei %.0f%% visų pastarųjų žaidimų",
    "Lyderių lentelė (paskutinė valanda)",
    "Žaidėjas",
    "Rezultatas",
    "Laikas",
    "Dar nėra rezultatų – būk pirmas!",
    "Bandymai: %d",
    "Diapazonas: %d–%d",
    "Žaidimo nustatymai",
    "Snaigės: %d / %d",
    "Snaigių tankis",
    "Sniegas visiškai išjungtas, įskaitant atnaujinimus ir piešimo iškvietimus.",
    "ATSTATYTI PROGRESĄ",
    "Atstato kampanijos lygį, atrakintus dalykus ir pinigus. Naudotojo vardas lieka nepakitęs.",
    "Mokykla :: Pamoka %d. %s",
    "Taikymo pamoka",
    "Kamuolio masė",
    "Kamuolio sukimas",
    "Alyva ir slydimas",
    "Strike'o linija",
    "MASĖ (%.1fKG)",
    "Lengvo kamuolio testas",
    "Sunkaus kamuolio testas",
    "Išlaikyta",
    "%d / %d pataikymai",
    "MASĖ",
    "SUKIMASIS",
    "SLYDIMAS",
    "SUKIBIMAS",
    "PAPRASTAS",
    "RETAS",
    "EPINIS",
    "LEGENDINIS",
    "Sukimosi testas: %d/%d monetų (Lygis %d/%d)",
    "Taikymo pamoka: %d/%d taškų",
    "Traukite kamuolį iki galo atgal",
    "Dabar paleiskite",
    "Eikite dešinėn",
    "Eikite kairėn",
    "Alyvos testas: pertepimai %d/%d",
    "Likusi alyva",
    "Pertepimo progresas",
    "Sudėvėkite alyvą prieš pertepdami.",
    "Strike testas: išmuškite STRIKE'ą",
    "1 LYGIS  PIRMASIS ŽINGSNIS",
    "Normalus biomas  Pasiek 100, kad pereitum",
    "2 LYGIS  MALACHAS ATVYKSTA",
    "Normalus biomas  Nugalėk Malachą",
    "3 LYGIS  DYKUMOS ĮSPĖJIMAS",
    "Dykumos biomas  Nugalėk Malachą",
    "4 LYGIS  STIKLINIS LEDAS",
    "Ledo biomas  Nugalėk Malachą",
    "5 LYGIS  ŠUNS IŠŠŪKIS",
    "Normalus biomas  Nugalėk Šunį",
    "6 LYGIS  JĖGOS SMŪGIO ALĖJA",
    "Neono biomas  Nugalėk Šunį",
    "7 LYGIS  SMĖLIO POKALBIS",
    "Dykumos biomas  Nugalėk Šunį",
    "8 LYGIS  SNAPAS KOPOSE",
    "Dykumos biomas  Nugalėk Snapą",
    "9 LYGIS  LEDO AUDITORIJA",
    "Ledo biomas  Nugalėk Snapą",
    "10 LYGIS  NEONO PRISIPAŽINIMAS",
    "Neono biomas  Nugalėk Snapą",
    "11 LYGIS  MIESTO RATAI",
    "Neono biomas  Nugalėk Karvę",
};

static constexpr const char *g_txl_chars_lt_lt = "MeniuPrvadtE įmokyląKbpjLsĮgŽGųAJDėščzRTIžSVU,YBNĄO%ę?.Šf:Ė(c)1/ū\n34$0!–'2CHh5Ū6789";

static constexpr const char *g_txl_jp_jp[TXL_KEY_COUNT] = {
    "メニュー",
    "名前変更",
    "学校へ行く",
    "言語",
    "キャンペーン",
    "練習",
    "フリースタイル",
    "デバイス共有",
    "ゲーム設定",
    "トラッカー",
    "言語選択",
    "英語",
    "中国語",
    "リトアニア語",
    "日本語",
    "サウンド",
    "曲",
    "オイル",
    "ショップ",
    "ハイスコア",
    "練習",
    "レーンとボールを選ぶ練習",
    "フリースタイル",
    "解除済みのエンジェル、レーン、ボールで対戦",
    "次のレベル",
    "もう一度",
    "ボールチャージ %d%%",
    "ボウリングを続けますか？",
    "このボウリングゲームへようこそ。",
    "「READY」をクリックしてゲーム画面にフォーカス。",
    "レディー",
    "エンジェルを選択",
    "対戦相手を選択",
    "レーンを選択",
    "レーンを切り替え",
    "ボールを選択",
    "今すぐ購入",
    "レーン",
    "選択中: %s",
    "レーンを選んでください",
    "レーンがありません",
    "対戦相手のアバター",
    "対戦相手のアバターを選択",
    "対戦相手がいません",
    "ショップ: ゲームを強化",
    "現在の残高",
    "ロック中",
    "%s にリセット",
    "サウンド設定",
    "品質を変更中...",
    "オーディオモード",
    "SDL バッファ",
    "音楽音量",
    "パフォーマンス低下を検出",
    "オプションを選択してください：",
    "シンセを使用",
    "キャッシュを使用",
    "無効にする",
    "シンセ：リアルタイムOPNチップ合成（プリロードなし、CPU負荷高め）",
    "キャッシュ：事前生成オーディオ（キャッシュ必要、CPU負荷低め）",
    "オーディオをキャッシュ中...",
    "進捗: %d%% (%.1f秒 / %.1f秒)",
    "オイル状態",
    "レーンオイルマップ",
    "トラック情報",
    "最大オイル量: %.1fmm\n現在: %.1fmm\nキャリーダウン: %.3fm/m\n減衰: %.4f/m",
    "滑りやすさ",
    "再オイル: 無料",
    "再オイル費用: $%.0f (所持金: $%.0f)",
    "再オイル: %d/%d",
    "再オイル",
    "資金不足",
    "再オイルはロックされています",
    "トップスコア",
    "あなたのスコアはトップ%dに入っています",
    "良いゲーム！",
    "%dポイント",
    "あなたのスコアは最近の全プレイの%.0f%%を上回っています",
    "リーダーボード（直近1時間）",
    "プレイヤー",
    "スコア",
    "時間",
    "まだスコアがありません - 最初に記録しよう！",
    "試行回数: %d",
    "範囲: %d～%d",
    "ゲーム設定",
    "雪片: %d / %d",
    "雪の密度",
    "雪は完全に無効（更新・描画を含む）",
    "進行状況をリセット",
    "キャンペーンレベル、アンロック、所持金をリセット。ユーザー名はそのまま。",
    "学校 :: レッスン %d. %s",
    "エイムレッスン",
    "ボールの質量",
    "ボールに回転をかける",
    "オイルとスキッド",
    "ストライクライン",
    "質量 (%.1fkg)",
    "軽いボールのテスト",
    "重いボールのテスト",
    "合格",
    "%d / %d ヒット",
    "質量",
    "回転",
    "スキッド",
    "バイト",
    "コモン",
    "レア",
    "エピック",
    "レジェンダリー",
    "スピンテスト: %d/%d コイン (レベル %d/%d)",
    "エイムレッスン: %d/%d ポイント",
    "ボールを一番後ろまで引いてください",
    "今すぐ放してください",
    "右に移動",
    "左に移動",
    "オイルテスト: 再オイル %d/%d",
    "残りのオイル",
    "再オイルの進行状況",
    "再オイルする前に使い切ってください。",
    "ストライクテスト: ストライクを取れ",
    "レベル1 最初の目標",
    "ノーマルバイオーム 100点でクリア",
    "レベル2 マラック登場",
    "ノーマルバイオーム マラックに勝利",
    "レベル3 砂漠の警告",
    "砂漠バイオーム マラックに勝利",
    "レベル4 ガラスの氷",
    "氷バイオーム マラックに勝利",
    "レベル5 ドッグの挑戦",
    "ノーマルバイオーム ドッグに勝利",
    "レベル6 パワーショットアレイ",
    "ネオンバイオーム ドッグに勝利",
    "レベル7 砂の会話",
    "砂漠バイオーム ドッグに勝利",
    "レベル8 砂丘のビーク",
    "砂漠バイオーム ビークに勝利",
    "レベル9 氷上の観客",
    "氷バイオーム ビークに勝利",
    "レベル10 ネオンの告白",
    "ネオンバイオーム ビークに勝利",
    "レベル11 街の車輪",
    "ネオンバイオーム カウに勝利",
};

static constexpr const char *g_txl_chars_jp_jp = "メニュー名前変更学校へ行く言語キャンペ練習フリスタイルデバ共有ゲム設定トラッカ選択英中国ア日本サウド曲オショプハコレとボをぶ解除済みのエジェ、で対戦次ベもう一度チ %dグ続けますか？こよそ。「READY」クして画面にォィ相手切り替え今ぐ購入:sんださいがあせ強化現在残高ロセ品質.モSLァ音楽量パマ低下検出：使用無効るOPN合成（なCU負荷め）事生必要進捗(1f秒/)状態情報最大m\nダ3減衰4滑や再料費$0所持金資不足はれたっ良！ポ近全上回直時間ヤ-初記録試数範囲～雪片密完新・描含む況ユザ転kg軽テ重格ヒピ番後ろ引放右移動左取目標ノ点2登場勝利砂漠警告ガ氷5挑6ワネ7会話8丘ビ9観客白街車輪";

static constexpr const char *g_txl_zh_cn[TXL_KEY_COUNT] = {
    "菜单",
    "改名",
    "去学校",
    "语言",
    "战役",
    "练习",
    "自由赛",
    "设备共享",
    "游戏设置",
    "音轨编辑",
    "选择语言",
    "English",
    "中文",
    "立陶宛语",
    "日语",
    "声音",
    "歌曲",
    "油道",
    "商店",
    "最高分",
    "练习",
    "选择球道和球的练习局",
    "自由赛",
    "使用已解锁的天使、球道和球对战",
    "下一关",
    "再试一次",
    "球电量 %d%%",
    "你想继续打保龄球吗？",
    "欢迎来到这款保龄球游戏。",
    "点击“准备”以聚焦游戏画面。",
    "准备",
    "选择天使",
    "选择对手",
    "选择球道",
    "切换球道",
    "选择球",
    "立即购买",
    "球道",
    "已选择：%s",
    "选择一个球道",
    "没有球道",
    "对手形象",
    "选择一个对手形象",
    "没有对手",
    "商店：强化这一局",
    "当前余额",
    "未解锁",
    "%s后重置",
    "声音设置",
    "正在切换音频模式...",
    "音频模式",
    "SDL 缓冲",
    "音乐音量",
    "检测到性能偏低",
    "请选择一个选项：",
    "使用合成",
    "使用缓存",
    "关闭音频",
    "合成：实时 OPN 芯片合成（无需预载，CPU 占用更高）",
    "缓存：预生成音频数据（需要缓存，CPU 占用更低）",
    "正在缓存音频...",
    "进度：%d%%（%.1f秒 / %.1f秒）",
    "油道状态",
    "油图",
    "球道信息",
    "最大油量：%.1f毫米\n当前：%.1f毫米\n带油：%.3f米/米\n衰减：%.4f/米",
    "滑度",
    "重新上油：免费",
    "重新上油费用：$%.0f（你有：$%.0f）",
    "重新上油：%d/%d",
    "重新上油",
    "余额不足",
    "暂时不能上油",
    "最高分",
    "你的分数进入前%d名",
    "不错的一局！",
    "%d分",
    "你的分数超过了近期%.0f%%的对局",
    "排行榜（最近一小时）",
    "玩家",
    "分数",
    "时间",
    "还没有分数，来当第一个吧！",
    "尝试次数：%d",
    "范围：%d-%d",
    "游戏设置",
    "雪花：%d / %d",
    "雪花密度",
    "雪已完全关闭，包括更新和绘制。",
    "重置进度",
    "重置战役关卡、解锁内容和现金。用户名保持不变。",
    "学校 :: 第%d课：%s",
    "瞄准课",
    "球的质量",
    "给球上旋",
    "油与滑行",
    "全中线路",
    "质量（%.1f公斤）",
    "轻球测试",
    "重球测试",
    "已通过",
    "%d / %d 次击中",
    "质量",
    "旋转",
    "滑行",
    "咬道",
    "普通",
    "稀有",
    "史诗",
    "传说",
    "旋转测试：%d/%d 宝石（第%d/%d关）",
    "瞄准课：%d/%d分",
    "把球尽量向后拉",
    "现在放手",
    "向右移动",
    "向左移动",
    "油道测试：重新上油 %d/%d",
    "剩余油量",
    "上油进度",
    "先把油耗掉，再重新上油。",
    "全中测试：打出STRIKE",
    "第1关  第一个里程碑",
    "普通球道  达到100分即可通过",
    "第2关  玛拉克到来",
    "普通球道  击败玛拉克",
    "第3关  沙漠警告",
    "沙漠球道  击败玛拉克",
    "第4关  玻璃冰面",
    "冰面球道  击败玛拉克",
    "第5关  狗的挑战",
    "普通球道  击败狗",
    "第6关  强力球球馆",
    "霓虹球道  击败狗",
    "第7关  沙中对话",
    "沙漠球道  击败狗",
    "第8关  沙丘中的喙",
    "沙漠球道  击败喙",
    "第9关  冰上会面",
    "冰面球道  击败喙",
    "第10关  霓虹告白",
    "霓虹球道  击败喙",
    "第11关  城市之轮",
    "霓虹球道  击败牛",
};

static constexpr const char *g_txl_chars_zh_cn = "菜单改名去学校语言战役练习自由赛设备共享游戏置音轨编辑选择English中文立陶宛日声歌曲油道商店最高分球和的局使用已解锁天、对下一关再试次电量 %d你想继续打保龄吗？欢迎来到这款。点击“准”以聚焦画面手切换即购买：个没有形象强化当前余额未后重正在频模式.SDL缓冲乐检测性能偏低请项合成存闭实时OPN芯片（无需预载，CU占更）生数据要进度1f秒/状态图信息大毫米\n带3衰减4滑新上免费$0不足暂入错！超过了近期排行榜小玩家间还第吧尝范围-雪花密完全包括绘制卡内容现金户持变:课瞄质给旋与线路公斤轻通转咬普稀史诗传说宝石把尽向拉放右移动左剩先耗掉出TRIK里程碑达可2玛克败沙漠警告玻璃冰5狗挑6力馆霓虹7话8丘喙9会白城市之轮牛";

